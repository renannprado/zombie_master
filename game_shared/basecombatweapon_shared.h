...
public:
	...
	//TGB: func that will be overridden in hl2mpbase, to get at the weapon script's damage
	virtual float			GetPlayerDamage() { return 0.0; }
...
public:
	...
	//LAWYER:  virtual function, sets the carry slot the weapon uses up
	virtual int				GetWeaponTypeNumber( void ) { return 0; }
...
// Server Only Methods
	...
	// ZM
	virtual bool			CanBeDropped() { return true; }			//certain ZM weapons can't be dropped, this lets us check without strcmp

	//TGB: found better solution
	//virtual Vector			GetBumpTracePos() { return EyePosition(); } //hack so we can fudge this for specific weapons that won't cooperate
...
public:
	//TGB: we want to store invisible ammo in the gun when we drop it, as it would otherwise remain in the player's inventory
	int						m_iStoredPrimaryAmmo; //only for primary ammo for now, I be lazy
