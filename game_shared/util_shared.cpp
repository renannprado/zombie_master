...
//-----------------------------------------------------------------------------
// Purpose: Trace filter that only hits anything but NPCs 
//-----------------------------------------------------------------------------
bool CTraceFilterNoNPCs::ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask )
{
	if ( CTraceFilterSimple::ShouldHitEntity(pServerEntity, contentsMask) )
	{
		CBaseEntity *pEntity = EntityFromEntityHandle( pServerEntity );
...
void UTIL_BloodDrips( const Vector &origin, const Vector &direction, int color, int amount )
{
	...
	//TGB: replaced with better fix from wiki
	//IPredictionSystem::SuppressHostEvents( NULL ); //qck: Blood fix. BLOOD FIX. BLOODBLOODBLOOD. 
	...
	//TGB: the amount of damage doesn't always get set properly in HL2MP
	if (amount == 0)
	{
		//can't be bothered to figure out a way to get the real damage set in all places
		//I doubt anyone will particularly care if the amount of blood spawned varies sometimes
		amount = random->RandomInt(40, 200);
	}
