...
{
	//if ( player->IsObserver() || (player->GetTeamNumber() == 3) )
	if ( player->IsObserver() ) //LAWYER:  Zombiemasters have similar rules
	{
...
{	
	//if ( player->IsObserver() || (player->GetTeamNumber() == 3) )
	if ( player->IsObserver())//LAWYER: Zombiemasters too!
	{
...
void CGameMovement::CheckParameters( void )
{
	...
	if ( player->GetMoveType() != MOVETYPE_ISOMETRIC &&
		 ...
		 player->GetMoveType() != MOVETYPE_CUSTOM && //LAWYER:  Custon is something to do with the ZM
	...
	if ( !IsDead() )
	{
		...
		if ( player->GetMoveType() != MOVETYPE_ISOMETRIC  &&
			 player->GetMoveType() != MOVETYPE_CUSTOM && //LAWYER:  Again, for the ZM
			 player->GetMoveType() != MOVETYPE_NOCLIP )
		{
...
//-----------------------------------------------------------------------------
// Purpose: This is the movement code for the Zombie Master -qck
//			
//-----------------------------------------------------------------------------
void CGameMovement::FullZombieMasterMove( float factor, float maxacceleration )
{
	Vector wishvel;
	Vector forward, right, up;
	Vector wishdir;
	...
	// No up / down movement when a ZombieMaster

	//TGB: is this team check necessary? I'm assuming this never gets called if you're not ZM
	if (player->GetTeamNumber() == 3 && player->m_bCameraMode == false) //LAWYER:  That need cleaning up
	{
		forward[2] = 0;
		right[2] = 0;
	}
	//LAWYER:  This magical piece of code makes sure that the camera doesn't spin like a crazyman in Zombie Master mode.
	...
	// Move with full world/object clipping -qck
	//TryZombieMasterMove(); Until this is fully operational, we should leave it out. 
	
	//Move without the clipping -qck
...
// Output : int
//-----------------------------------------------------------------------------
int CGameMovement::TryZombieMasterMove( Vector *pFirstDest, trace_t *pFirstTrace )
{
	...
	for (bumpcount=0 ; bumpcount < numbumps; bumpcount++)
	{
		...
		if ( g_bMovementOptimizations )
		{
			...
			if ( pFirstDest && end == *pFirstDest )
				...
				TracePlayerBBox( mv->m_vecAbsOrigin, end, MASK_NPCWORLDSTATIC, COLLISION_GROUP_PLAYER_MOVEMENT, pm );
		}
		else
		{
			TracePlayerBBox( mv->m_vecAbsOrigin, end, MASK_NPCWORLDSTATIC, COLLISION_GROUP_PLAYER_MOVEMENT, pm );
...
bool CGameMovement::LadderMove( void )
{
	...
	if ( (player->GetMoveType() == MOVETYPE_NOCLIP) || player->GetTeamNumber() == 3 || player->GetTeamNumber() == 1) //LAWYER:  Not for trixy ZMs, or observers!!
		return false;

	if ( player->m_bHoldingObject )
		return false;
	...
	else
	{
		...
		if ( pm.plane.normal[2] < 0.7)
		{
			...
			if ( pm.plane.normal[2] < 0.7)
			{
				...
				if ( ( mv->m_vecVelocity.z > 0.0f ) && ( player->GetMoveType() != (MOVETYPE_NOCLIP | MOVETYPE_CUSTOM))) //LAWYER:  More ZM move stuff?
				{
...
void CGameMovement::Duck( void )
{
	...
	//TGB: ZM no ducky ducky. We use the key for typeselect
...
void CGameMovement::PlayerMove( void )
{
	...
	switch (player->GetMoveType())
	{
		...
		case MOVETYPE_CUSTOM:
			FullZombieMasterMove( sv_noclipspeed.GetFloat(), sv_noclipaccelerate.GetFloat()  );
			break;
