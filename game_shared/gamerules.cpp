...
static CViewVectors g_DefaultZMViewVectors(
...
bool CGameRules::CanHaveAmmo( CBaseCombatCharacter *pPlayer, int iAmmoIndex )
{
	if ( iAmmoIndex > -1 )
	{
		...
		//TGB: check if we have the matching weapon
		CBaseCombatWeapon *weapon = pPlayer->Weapon_GetWpnForAmmo( iAmmoIndex );
		if ( weapon == NULL )
			return false; //nope, no weapon for this ammo
		
		//TGB: no pickup if the shotgun/rifle is currently reloading (picking up for non-active weapons should still work)
		if ( weapon->m_bReloadsSingly && weapon->m_bInReload )
			return false;
...
bool CGameRules::IsSpawnPointValid( CBaseEntity *pSpot, CBasePlayer *pPlayer  )
{
	...
	//TGB: 128 units? what the hells? players are only 33 wide
	for ( CEntitySphereQuery sphere( pSpot->GetAbsOrigin(), 20 /*128*/ ); (ent = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
...
const CViewVectors* CGameRules::GetZMViewVectors() const
{
	return &g_DefaultZMViewVectors;
}
