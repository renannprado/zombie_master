...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	for (int iShot = 0; iShot < info.m_iShots; iShot++)
	{
		...
		//TGB: we want all shotgun bullets to be bullets, not hulls
		//		that way we can let the shotgun have a smaller cone without overpowering
		...
		if (tr.fraction != 1.0)
		{
			...
			{
				//TGB: too restrictive for shotgun
				//UNDONE: alwaysgib now triggers crazy explosion effect, shouldn't happen easily
				//nActualDamageType = nDamageType | ((flActualDamage > 16) ? DMG_ALWAYSGIB : DMG_NEVERGIB );
				//nActualDamageType = nDamageType | ((flActualDamage < 6) ? DMG_ALWAYSGIB : DMG_NEVERGIB );
				nActualDamageType = nDamageType | DMG_NEVERGIB;
			}
...
void CBaseEntity::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( m_takedamage )
	{
		...
		if ( blood != DONT_BLEED )
		{
			//TGB: info.GetDamage doesn't give us a useful damage value as those fields are not filled in AddAmmoType calls
...
void CBaseEntity::TraceBleed( float flDamage, const Vector &vecDir, trace_t *ptr, int bitsDamageType )
{
	...
	trace_t Bloodtr;
	Vector vecTraceDir;
	Vector vecBloodSplash;
	trace_t BloodFeet_tr;
	float flNoise;
	int cCount;
	int i;
...
//#endif
	...
	for ( i = 0 ; i < cCount ; i++ )
	{
		...
		if ( Bloodtr.fraction != 1.0 )
		{
			...
			//debugoverlay->AddLineOverlay(ptr->endpos, ptr->endpos + vecDir * 72, 255, 255, 255, true, 20.0); Visualize our trace
		}
	}

	

	//qck: spawn a bit of blood on the ground near the zombies feet to simulate drips landing. It's fun!
#ifdef CLIENT_DLL
	vecBloodSplash = vecDir * -1;
	
	vecBloodSplash.z += random->RandomFloat( 0 , 3 );
	vecBloodSplash.y += random->RandomFloat( -0.5, 0.5 );
	vecBloodSplash.x += random->RandomFloat( -0.5, 0.5 );

	AI_TraceLine( ptr->endpos, ptr->endpos + vecBloodSplash * -flTraceDist, MASK_SOLID_BRUSHONLY & ~CONTENTS_GRATE, this, COLLISION_GROUP_NONE, &BloodFeet_tr);

	if ( BloodFeet_tr.fraction != 1.0 )
	{
		UTIL_BloodDecalTrace( &BloodFeet_tr, BLOOD_COLOR_RED ); //TGB decrees: we will never have dudes that bleed green
		//debugoverlay->AddLineOverlay(ptr->endpos, ptr->endpos + vecBloodSplash * 72, 255, 255, 255, true, 20.0); //Visualize our trace
	}
#endif
