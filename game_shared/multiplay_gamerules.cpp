...
#define INTERMISSION_PERIOD 3.0f //LAWYER:  How long before a round ends!
...
ConVar	zm_votesneeded( "zm_votesneeded",
					  "0.75",
					  FCVAR_NOTIFY|FCVAR_REPLICATED,
					  "percentage of votes needed to cause a round restart" );
#ifdef GAME_DLL
...
//TGB: cvar time!
ConVar zm_resource_refill_min( "zm_resource_refill_min", "25", FCVAR_NOTIFY|FCVAR_REPLICATED, "Minimum resource refill amount" );
ConVar zm_resource_refill_max( "zm_resource_refill_max", "75", FCVAR_NOTIFY|FCVAR_REPLICATED, "Maximum resource refill amount" );

ConVar zm_resource_refill_rate( "zm_resource_refill_rate", "5", FCVAR_NOTIFY|FCVAR_REPLICATED, "Seconds between resource refills" );
ConVar zm_kill_reward( "zm_kill_reward", "150", FCVAR_NOTIFY|FCVAR_REPLICATED, "How much resources the ZM gets for killing a player" );
ConVar zm_resource_limit( "zm_resource_limit", "4000", FCVAR_NOTIFY|FCVAR_REPLICATED, "The max number of resources the ZM can hoard.");
//TGB: obsolete
//#define POOL_REFILL_RATE 5.0f
...
bool CMultiplayRules::IsMultiplayer( void )
{
	...
	#define WEAPON_RESPAWN_TIME	600 //TGB: I do wonder if these are actually used, upped just in case
	...
	{
		//LAWYER:  Initialise the round restart variable
		g_pGameRules->m_flRoundRestart = 0.0f;
		...
		//LAWYER:  Do our banlist here.  As people shouldn't really fiddle with it, I'm not going to CVAR it
		Msg( "Executing default banlist config file\n" );
		engine->ServerCommand("exec defaultbanlist.cfg\n");
	}
	...
	bool CMultiplayRules::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon )
	{
		...
		if ( pPlayer->m_bHoldingObject == true )
		{
	...
	bool CMultiplayRules::ClientConnected( edict_t *pEntity, const char *pszName, const char *pszAddress, char *reject, int maxrejectlen )
	{
		...
		/* TGB: moved to PlayerSpawning
		#ifndef CLIENT_DLL
				CTeam *ZMteam = GetGlobalTeam(3);
				CTeam *Survivorteam = GetGlobalTeam(2);

				if ( ZMteam && Survivorteam)
				{
					if (ZMteam->GetNumPlayers() < 1 || Survivorteam->GetNumPlayers() < 1)
					{
						//Restart the round if we find ourselves without a Zombiemaster!
						//TGB: or if the ZM is all alone without survivors to kill
						g_pGameRules->FinishingRound(); 
					}
				}
				else
					Warning("Teams not found!\n");
	...
	void CMultiplayRules::ClientDisconnected( edict_t *pClient )
	{
		if ( pClient )
		{
			...
			if ( pPlayer )
			{
				pPlayer->ChangeTeam(TEAM_UNASSIGNED); //LAWYER:  Strip him from his team!
			...
			} //LAWYER: We need checks for Zombie Masters here
			#ifndef CLIENT_DLL
				CTeam *team = GetGlobalTeam(3);
				CTeam *team2 = GetGlobalTeam(2);
				if ( team )
				{
					if (team->GetNumPlayers() < 1)
					{
						g_pGameRules->FinishingRound(); //Restart the round if we find ourselves without a Zombiemaster!
					}
				}
				...
				if ( team2 )
				{
					...
					{
						g_pGameRules->FinishingRound(); //Restart the round if we find ourselves without a human team!
					}
				}
	...
	{ //LAWYER:  We do our resource increments here!
//			Warning("Thinking...");
			#ifndef CLIENT_DLL
			//TGB: added a 9900 res cap here to prevent HUD overlaps. Above that increments stop.
			if ( gpGlobals->curtime > pPlayer->m_fNextIncrement && pPlayer->m_iZombiePool < zm_resource_limit.GetInt() )
			{
//				Warning("Do Increment\n");
				//CTeam *team = GetGlobalTeam(2);
				//TGB: get number of survivors + observers instead
				//if there's a way to just grab global playercount, that would be better
				int numplayers = GetGlobalTeam(2)->GetNumPlayers() + GetGlobalTeam(1)->GetNumPlayers();


				int increment = (int)SimpleSplineRemapVal( numplayers, 1, 15, zm_resource_refill_min.GetInt(), zm_resource_refill_max.GetInt() );

				//TGB: messy but handy
				/*
				const int min = zm_resource_refill_min.GetInt();
				const int max = zm_resource_refill_max.GetInt();
				DevMsg("incr1-5: %i - %i - %i - %i - %i\n", (int)SimpleSplineRemapVal( 1, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 2, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 3, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 4, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 5, 1, 16, min, max ) );
				DevMsg("incr6-10: %i - %i - %i - %i - %i\n", (int)SimpleSplineRemapVal( 6, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 7, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 8, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 9, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 10, 1, 16, min, max ) );
				DevMsg("incr11-15: %i - %i - %i - %i - %i\n", (int)SimpleSplineRemapVal( 11, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 12, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 13, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 14, 1, 16, min, max ),
													(int)SimpleSplineRemapVal( 15, 1, 16, min, max ) );
				*/

				//pPlayer->m_iZombiePool += ((numplayers + 3) * 5);
				pPlayer->m_iZombiePool += increment;

				pPlayer->m_fNextIncrement = gpGlobals->curtime + zm_resource_refill_rate.GetFloat();
			}
			#endif
		if ( g_fGameOver )
		{
	...
	void CMultiplayRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		else if ( pScorer )
		{
			...
			//LAWYER:  Announce SteamID of TKs
			if (pScorer->GetTeamNumber() == 2)
			{
				UTIL_ClientPrintAll( HUD_PRINTCONSOLE, "SteamID %s1 (%s2) teamkilled.\n", pScorer->GetNetworkIDString(), pScorer->GetPlayerName());
		...
		{  
			...
			//pVictim->IncrementFragCount( -1 );  //LAWYER:  No you don't!


		}
			// loop through all clients

		//LAWYER:  A player died!  If it isn't the Zombie master, award some points!
		if (pVictim->GetTeamNumber() == 2)
		{
			...
			for ( int i = 1; i <= gpGlobals->maxClients; i++ )
			{
				...
				if ( plr )
				{
					...
					{
							plr->m_iZombiePool += zm_kill_reward.GetInt(); //Lots more resources for doing it
							plr->IncrementFragCount(10); //LAWYER:  10 points for killing a player
							Msg("%s has been killed\n", pVictim->GetPlayerName());
	...
	if (g_pGameRules->m_flRoundRestart == 0.0f) //LAWYER:  The round isn't finished.
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			CBasePlayer *plr = UTIL_PlayerByIndex( i );
			...
			if ( plr )
			{
				if (plr->GetTeamNumber() == 2)
				{
					if (plr->GetHealth() > 0)
					{
						//LAWYER:  If a player isn't dead, don't restart the round!
						return;
					}
				}
			}
		...
		/*char text[256];
		Q_snprintf( text,sizeof(text), "Undeath has prevailed!\n" );*/

		UTIL_ClientPrintAll( HUD_PRINTTALK, "Undeath has prevailed!\n" );

		HL2MPRules()->TeamVictorious( false, "humans_dead" );

		g_pGameRules->FinishingRound(); //LAWYER:  End the round!
	}
}
	...
	*/
	...
	bool CMultiplayRules::PlayFootstepSounds( CBasePlayer *pl )
	{
		...
		if ( pl->IsZM() ) //tGB: post-merge zm made footstepsounds
			return false;
	...
	void CMultiplayRules::FinishingRound()
	{
		//LAWYER:  ZOOOOMJ! There's been a call to restart the round!  Start the clock!
		if (g_pGameRules->m_flRoundRestart <= gpGlobals->curtime) //LAWYER:  Only change the clock if it's not ticking
		{
			g_pGameRules->m_flRoundRestart = gpGlobals->curtime + 4.0f;
	...
	void CMultiplayRules::CheckRoundRestart()
	{
		//LAWYER:  People are thinking of restarting the round...
		float fCountVotes = 0; //Count the votes, beyotch
		float fCountPlayers = 0; //Count the players too
			for ( int i = 1; i <= gpGlobals->maxClients; i++ )
			{
				CBasePlayer *pPlayer = UTIL_PlayerByIndex( i );
				...
				if ( pPlayer && pPlayer->m_iZMVoteRR == 1)
				{
					fCountVotes++;
				}
		...
		//Okay, count it up
/*			Warning("Votes: %f\n", fCountVotes);
			Warning("Total players: %f\n", fCountPlayers);
			Warning("Percent required: %f\n", zm_votesneeded.GetFloat());
			Warning("Percent achieved: %f\n", fCountVotes/fCountPlayers);*/
		if (fCountVotes/fCountPlayers >= zm_votesneeded.GetFloat())
		{
			//We've got enough.  ZOOOMJ!
			char text[256];
			Q_snprintf( text,sizeof(text), "%f percent of players have voted for a new round to begin.\n", fCountVotes/fCountPlayers  );
	...
	//--------------------------------------------------------------
	// TGB: a player has spawned, so see if we need to roundrestart 
	//--------------------------------------------------------------
	void CMultiplayRules::PlayerSpawning( void )
	{
		//TGB: the reason I added this is that ClientConnected would do a roundrestart even for
		//	players who connected but couldn't join due to networking tables, etc. Basically
		//	ClientConnected is too early to do the RR in.
#ifndef CLIENT_DLL
		CTeam *ZMteam = GetGlobalTeam(3);
		CTeam *Survivorteam = GetGlobalTeam(2);
