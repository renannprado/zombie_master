...
void EventList_RegisterSharedEvents( void )
{
	...
	//TGB: Melee hit event for our melee weapon attack timings
	//TGB: I've made minor tweaks to event firing so that AE_TYPE_SHARED now fires on both server and client, as it should
	REGISTER_SHARED_ANIMEVENT( AE_ZM_MELEEHIT, AE_TYPE_SHARED );
	REGISTER_SHARED_ANIMEVENT( AE_ZM_REVOLVERSHOOT , AE_TYPE_SHARED ); //LAWYER:  Revolvering
	
	REGISTER_SHARED_ANIMEVENT( AE_ZM_BEAMON, AE_TYPE_SERVER);
	REGISTER_SHARED_ANIMEVENT( AE_ZM_BEAMOFF, AE_TYPE_SERVER);
	REGISTER_SHARED_ANIMEVENT( AE_ZM_LIGHTERFLAME, AE_TYPE_SERVER ); //maybe make this client? hrm I fear server/client troubles with it
