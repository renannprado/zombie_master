...
void CBasePlayer::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
	...
	if ( !pVehicle )
	{
		...
		{
			//LAWYER:  Flatten our EyeAngles once and for all!
			eyeAngles[ROLL] = 0;
...
void CBasePlayer::CalcObserverView( Vector& eyeOrigin, QAngle& eyeAngles, float& fov )
{
	...
	//TGB: if we lose our target, server has to reset viewangles as well
...
void CBasePlayer::SharedSpawn() //LAWYER:  Where we change stuff particular to players
{
	...
	if(GetTeamNumber() == 2) //LAWYER:  Bounding box stuff for non-ZMs
	{
		if ( GetFlags() & FL_DUCKING ) 
	...
	//LAWYER:  Add checks for team here!
	if (this->GetTeamNumber() == 3) //LAWYER:  We really need to change this to the shared define 
	{
...
//TGB: ZM check
bool CBasePlayer::IsZM () 
{
	return (GetTeamNumber() == 3);
}

bool CBasePlayer::IsSurvivor () 
{
	return (GetTeamNumber() == 2);
}