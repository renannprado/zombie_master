...
void CVoiceStatus::DrawHeadLabels()
{
	...
	for(int i=0; i < VOICE_MAX_PLAYERS; i++)
	{
		...
		if( pPlayer->IsPlayerDead() || (pPlayer->GetTeamNumber() == 3)) //LAWYER:  No voice icon for the ZM
			continue;
...
void CVoiceStatus::SetPlayerBlockedState(int iPlayer, bool blocked)
{
	...
	if (IsPlayerAudible(iPlayer)) //TGB: made into proper check to fit our audibleplayers policy better
	{
		if (voice_clientdebug.GetInt())
		{
