...
protected:
	...
	// Handle movement as the Zombie Master - qck
	void			FullZombieMasterMove( float factor, float maxacceleration  );
	...
	// qck: Movement clip with custom zm traces
	virtual int		TryZombieMasterMove( Vector *pFirstDest=NULL, trace_t *pFirstTrace=NULL );
...
inline void CGameMovement::TracePlayerBBox( const Vector& start, const Vector& end, unsigned int fMask, int collisionGroup, trace_t& pm )
{
	...
	//TGB: did a quick try of making players clip through each other, but it weirded out. Better to wait until TF2's code is put up somewhere
	Ray_t ray;
