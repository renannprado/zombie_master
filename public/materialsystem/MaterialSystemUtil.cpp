...
//TGBMERGENOTE: qck uses this for riflestuff. I don't know if we should be using the above now instead.
// qck edit- This lets us create named render targets CORRECTLY
void CTextureReference::InitNamedRenderTarget( const char *pRTName, int w, int h, RenderTargetSizeMode_t sizeMode, ImageFormat fmt, MaterialRenderTargetDepth_t depth )
{
	Shutdown();

	m_pTexture = materials->CreateNamedRenderTargetTexture( pRTName, w, h, sizeMode, fmt, depth );
#ifdef _DEBUG
