...
mstudioseqdesc_t &CStudioHdr::pSeqdesc( int i ) const
{
	...
	//TGB: this stuff should not be breaking. Why is it breaking?
	//	all we can do is patch the holes and hope the sea goes away
	if (pStudioHdr)
		return *pStudioHdr->pLocalSeqdesc( m_pVModel->m_seq[i].index );
	else
		return *m_pStudioHdr->pLocalSeqdesc( i ); //hope this works in this situation, it's our only option
}
