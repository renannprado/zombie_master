...
void CTempEnts::MuzzleFlash_SMG1_NPC( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	...
	int iColor = random->RandomInt(225,255);
	color32 color;
	color.r = iColor;
	color.g = iColor;
	color.b = iColor;
	...
	//FX_Smoke( const Vector &origin, const QAngle &angles, float scale, int numParticles, unsigned char *pColor, int iAlpha )
	FX_Smoke( origin, angles, random->RandomFloat( 0.3, 1), random->RandomInt( 0, 2 ), (unsigned char *)&color, random->RandomInt( 50, 128 ) );
}
...
void CTempEnts::MuzzleFlash_Shotgun_NPC( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	...
	//FX_Smoke( const Vector &origin, const QAngle &angles, float scale, int numParticles, unsigned char *pColor, int iAlpha )
	FX_Smoke( origin, angles, random->RandomFloat( 0.4, 0.8), random->RandomInt( 0, 2 ), (unsigned char *)&color, random->RandomInt( 64, 128 ) );
...
//TGB: we want this so we can make nice rifle/revolver smoke too
void CTempEnts::MuzzleFlash_357_NPC( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	FX_MuzzleEffectAttached( 0.8f, hEntity, attachmentIndex, NULL, true );
	...
	//FX_Smoke( const Vector &origin, const QAngle &angles, float scale, int numParticles, unsigned char *pColor, int iAlpha )
	FX_Smoke( origin, angles, random->RandomFloat( 0.75, 1.5), random->RandomInt( 1, 3 ), (unsigned char *)&color, random->RandomInt( 64, 128 ) );
...
void CTempEnts::MuzzleFlash_Pistol_NPC( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	...
	//FX_Smoke( const Vector &origin, const QAngle &angles, float scale, int numParticles, unsigned char *pColor, int iAlpha )
	FX_Smoke( origin, angles, random->RandomFloat( 0.5, 1), random->RandomInt( 0, 2 ), (unsigned char *)&color, random->RandomInt( 64, 128 ) );
