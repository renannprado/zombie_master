...
//extern ConVar r_FlashlightIsOn;		// qck edit
//extern bool rifleZoomed; //qck: I have hated this from day one, but it's needed because of a holster code bug
...
IMPLEMENT_CLIENTCLASS_DT(C_HL2MP_Player, DT_HL2MP_Player, CHL2MP_Player)
	...
	RecvPropFloat( RECVINFO( m_flFlashlightBattery ) ),
	RecvPropEHandle( RECVINFO( m_hRagdoll ) ),
	RecvPropInt( RECVINFO( m_iSpawnInterpCounter ) ),
	RecvPropInt( RECVINFO( m_iPlayerSoundType) ),

	RecvPropInt( RECVINFO( m_zmParticipation )),
END_RECV_TABLE()
...
//#define FADE_TIME	10.0f //LAWYER:  Redundant
...
//TGB: firstperson ragdoll convars
...
static ConVar cl_ragdoll_fadetime ("cl_ragdoll_fadetime", "10", FCVAR_ARCHIVE, "Time until a ragdoll is removed from the game"); //LAWYER: Ragdoll Fade Time
...
//-----------------------------------------------------------------------------
void C_HL2MP_Player::UpdateIDTarget() //LAWYER:  This could help in the ID thing
{
	if ( !IsLocalPlayer() )
		return;
...
{
	//qck: CONSOLESPAMTIME
	//DevMsg("Screen effect: %s\n", m_ScreenEffect);

	if((this->IsLocalPlayer()) && ((this->GetTeamNumber()) == 2))
	{
		////if we're dead and we happen to be zoomed in with the Rifle turn off the Rifle shader -qck
		//if( !IsAlive() )
		//{
		//	rifleZoomed = false;
		//}

		//// if we're not holding the Rifle make sure the Rifle shader is off -qck
		//if ( GetActiveWeapon() )
		//{
		//	CBaseCombatWeapon *pRifle = Weapon_OwnsThisType("weapon_zm_rifle");

		//	if ( !(GetActiveWeapon() == pRifle) )
		//	{
		//		rifleZoomed = false;
		//	}

			////The flashlight is off if we're not holding it
			//CBaseCombatWeapon *pFlashlight = Weapon_OwnsThisType("weapon_zm_flashlight");

			//if ( !(GetActiveWeapon() == pFlashlight) )
			//{
	...
	for( int iClient = 1; iClient <= gpGlobals->maxClients; ++iClient )
	{
		...
		//TGB: was: !pEnt->IsPlayer(), but I'd say we can look at zombies as well
		if(!pEnt || (pEnt->IsPlayer() || pEnt->IsNPC()) == false )
			continue;

		if ( pEnt->entindex() == entindex() )
			continue;

		//TGB: don't look at players from non-survivor teams
		if ( pEnt->GetTeamNumber() != 2 )
			continue;
...
ConVar zm_simpleflashlight( "zm_simpleflashlight", "0", FCVAR_ARCHIVE, "Enables a simpler flashlight that works with shadows disabled." );
...
void C_HL2MP_Player::AddEntity( void )
{
	...
	//if this dude is not us (ie. third person) or we turned on simpleflashlight due to crappy pc
	if( this != C_BasePlayer::GetLocalPlayer() || zm_simpleflashlight.GetBool() )
	{
		//TGB: merging back to hl2dm flashlight until better option comes along

		//TGB: thirdperson light effect is always on if the flashlight is out
		//if ( GetActiveWeapon() && Q_strcmp(GetActiveWeapon()->GetClassname(), "weapon_zm_flashlight" ) == 0 )
		if ( IsEffectActive( EF_DIMLIGHT ) )
		{

			/* TGB: old custom flashlight code
			const int attachment = GetActiveWeapon()->LookupAttachment( "light" );
			if (attachment == 0)
			{
			...
			GetActiveWeapon()->GetAttachment( attachment, vecAttachment, vecAngle );
			AngleVectors ( vecAngle, &vecDir );
				
			trace_t tr;
			UTIL_TraceLine( vecAttachment, vecAttachment + (vecDir * 400), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );
			*/
			...
			trace_t tr;
			UTIL_TraceLine( vecOrigin, vecOrigin + (vForward * 250), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );
			...
			if( !m_pFlashlightBeam )
			{
				...
				beamInfo.m_flFadeLength = 200.0f;
				beamInfo.m_flAmplitude = 0;
				beamInfo.m_flBrightness = 10.0f;
...
{
	return false; //LAWYER:  No sprinting
...
{
	//TGB: credit and thanks to Wunderboy for posting much of this on the valve devwiki, and Dutchmega for adding the autoswitch bit
...
{
	//m_fFadeFrame = gpGlobals->curtime + FADE_TIME; //LAWYER:  Fade in ten seconds
	m_fFadeFrame = gpGlobals->curtime + cl_ragdoll_fadetime.GetFloat(); //LAWYER:  Moved to Convar
}
...
extern void SpawnBlood(Vector vecSpot, const Vector &vecDir, int bloodColor, float flDamage); //qck: For BLOOD!
...
void C_HL2MPRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, char *pCustomImpactName )
{
	...
	if ( iDamageType == DMG_BLAST )
	{
		...
		FX_Blood( pTrace->endpos, dir, 230, 5, 5, 230);	
		SpawnBlood(pTrace->endpos, dir, BloodColor(), 100); //qck: Drips of blood
		TraceBleed( 100, dir, pTrace, iDamageType ); //qck: splats of blood
		UTIL_BloodImpact( pTrace->endpos, dir, BloodColor(), 100);
	}
	else
	{
		Vector hitpos;  
...
//LAWYER:  Extra functions to handle ragdoll deletion
...
*/
	if (gpGlobals->curtime > m_fFadeFrame)
	{
		FadeOut();
	}
}

void C_HL2MPRagdoll::FadeOut( void ) //LAWYER:  We need to have checks for how long a Ragdoll has been in the world
{
