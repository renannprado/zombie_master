...
{
	//DevMsg("SB: Scoreboard constructed\n");
}
...
void CHL2MPClientScoreBoardDialog::InitScoreboardSections()
{
	...
	{
		//DevMsg("SB: Adding sections\n");
		// add the team sections
		AddSection( TYPE_TEAM, TEAM_HUMANS );
		AddSection( TYPE_TEAM, TEAM_ZOMBIEMASTER );
		AddSection( TYPE_TEAM, TEAM_SPECTATOR ); //LAWYER:  Observers exist on the scoreboard.
	}
	else
	{
...
void CHL2MPClientScoreBoardDialog::UpdateTeamInfo()
{
	...
	for ( int i = TEAM_SPECTATOR; i < TEAM_MAXCOUNT; i++ )
	{
		...
		if ( team )
		{
			...
			{
				//DevMsg("SB: Updating teaminfo on scoreboard\n");
...
{
	//DevMsg("SB: Adding scoreboard header\n");
...
int CHL2MPClientScoreBoardDialog::GetSectionFromTeamNumber( int teamNumber )
{
	...
	case TEAM_ZOMBIEMASTER:
		return SCORESECTION_ZOMBIEMASTER;
	case TEAM_SPECTATOR:
...
void CHL2MPClientScoreBoardDialog::UpdatePlayerInfo()
{
	...
	for ( i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( shouldShow )
		{
			...
			if (g_PR->GetZMImportance( i ) == 1) //LAWYER:  Give cool colours 
			{
				m_pPlayerList->SetItemFgColor( itemID, Color(87,0,255,255));
			}
			else if (g_PR->GetZMImportance( i ) == 2) //LAWYER:  Tester colours
			{
				m_pPlayerList->SetItemFgColor( itemID, Color(43,255,255,255));
			}
			else if (g_PR->GetZMImportance(i) == 3) //blue
			{
				m_pPlayerList->SetItemFgColor( itemID, Color(50,60,255,255));
			}
			else
			{
