...
void CTeamPlayHud::OnThink()
{
	...
	if (pLocalPlayer->GetTeamNumber() == TEAM_SPECTATOR)  //LAWYER:  Team labels aren't drawn for spectators
		return;
	...
	if ( pTeam )
	{
		...
		//m_pWarmupLabel->SetVisible( true ); TGB: never show this

		m_pWarmupLabel->SizeToContents();

		//SetVisible( true ); TGB: never show this
	}
