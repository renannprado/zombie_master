...
void CHudDeathNotice::FireGameEvent( IGameEvent * event )
{
	...
	bool zombiekill = event->GetBool("zombie");
	...
	if ( !killer_name )
		killer_name = "";
	//TGB: players killed by zombie-thrown physics tend to get a message saying they're killed by 'unconnected', so we change this to 'something'
	...
	deathMsg.iSuicide = ( /*!killer ||*/ killer == victim || Q_strcmp( killedwith, "worldspawn" ) == 0 );
	...
	DevMsg("Checking for zombiekill\n");
	//TGB: handle zombie kills with custom death msg stuff
	if (zombiekill)
	{
		DevMsg("DeathNotice detects zombiekill. killedwith=%s\n", killedwith );
		//the zombie's name is sent in killedwith/weapon
