...
#define	PARTICLE_LIFETIME	0.2f //qck: For large amounts of smoke
#define	MIN_SPEED			32
#define	MAX_SPEED			64
	...
	//if ( ShouldEmit() )
	{
		...
		for ( int i = 1; i < numPuffs+1; i++ )
		{
			...
			{
				pParticle->m_flLifetime		= 0.0f;
				pParticle->m_flDieTime		= random->RandomFloat(0.5f, 0.8f); //qck: Flame height variation. I'd make them taller, but sorting issues really start to arise then.
				...
				//qck: Some variation in color
				
				pParticle->m_uchColor[0]	= 255.0f;
				pParticle->m_uchColor[1]	= random->RandomFloat( 245.0f, 255.0f);
				pParticle->m_uchColor[2]	= random->RandomFloat( 100.0f, 255.0f);
				
				//TGB: no need to multiply stuff here now that we have networked vars to control them with
				pParticle->m_uchStartSize	= m_StartSize /* * 2.0f*/;
				pParticle->m_uchEndSize		= m_EndSize /* * 0.5f */;
				
				pParticle->m_uchStartAlpha	= 255; 
				//pParticle->m_uchEndAlpha	= 0;
				
				pParticle->m_flRoll			= random->RandomInt( 0, 360 ); //qck edit
		...
		offset = RandomVector( -m_StartSize*0.5f, m_StartSize*0.5f ) + GetAbsOrigin();
		...
		{
			pParticle->m_flLifetime		= 0.0f;
			pParticle->m_flDieTime		= ( PARTICLE_LIFETIME * 10.0f ) + random->RandomFloat(PARTICLE_LIFETIME*0.75f, PARTICLE_LIFETIME*1.65f);
