...
//	virtual int		GetPacketloss( int index );
	...
	virtual int		GetZMPriority( int index );
	virtual int		GetZMVoteRR( int index );
	virtual int		GetZMImportance( int index );
	//virtual int		GetSteamID( int index ); //LAWYER:  Needed for scoreboard shenanigans
protected:
	...
	int		m_iZMPriority[MAX_PLAYERS+1];
	int		m_iZMVoteRR[MAX_PLAYERS+1];
	int		m_iZMImportance[MAX_PLAYERS+1];
};
