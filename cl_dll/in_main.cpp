...
ConVar cl_pitchspeed( "cl_pitchspeed", "100", 0 ); //TGB: was: 225, which is high for ZMstuff, should move this into an option panel sometime
...
//TGB: radio commands menu toggle functions, fairly direct rips from scoreboard
//down...
void IN_RadioDown(void)
{
	KeyDown(&in_radio);
	DevMsg("Radio down.\n");
	...
	if ( gViewPortInterface )
	{
		gViewPortInterface->ShowPanel( PANEL_RADIO, true );
	}
}

//... and up
void IN_RadioUp(void)
{
	KeyUp(&in_radio);
	DevMsg("Radio up.\n");
	...
	if ( gViewPortInterface )
	{
		gViewPortInterface->ShowPanel( PANEL_RADIO, false );
		GetClientVoiceMgr()->StopSquelchMode();
	}
}
...
{
		//Took care of the if statement checking for the value of cl_mouselook. This allows us to use AdjustPitch.
	    //I couldn't just change the value of it because of the way the projects are structured - qck
		float	up, down;
...
{	
	...
	// Get current view angles after the client mode tweaks with it
	cmd->buttons |= IN_VALIDVGUIINPUT; //LAWYER: VGUI screen fix
	engine->SetViewAngles( cmd->viewangles );
...
int CInput::GetButtonBits( int bResetState )
{
	...
	//TGB: for ZM	in_radio
	CalcButtonBits( bits, IN_RADIO, s_ClearInputState, &in_radio, bResetState );
...
//ZM commands
//
static ConCommand startradio("+radio", IN_RadioDown);
static ConCommand endradio("-radio", IN_RadioUp);
