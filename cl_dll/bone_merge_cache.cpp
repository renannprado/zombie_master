		...
		if ( pTestFollow && pTestHdr && pOwnerHdr )
		{
			...
			for ( int i = 0; i < m_pOwnerHdr->numbones(); i++ )
			{
				...
				if ( ( m_pFollowHdr->pBone( parentBoneIndex )->flags & BONE_USED_BY_BONE_MERGE ) == 0 )
				{
					...
					DevWarning("Performance warning: Mark bone '%s' in model '%s' as being used by bone merge in the .qc!\n",
						m_pFollowHdr->pBone( parentBoneIndex )->pszName(), m_pFollowHdr->pszName() ); //TGB: no need to spam this to non-devs
				}
			}
