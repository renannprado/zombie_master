...
static ConVar  cl_interp	 ( "cl_interp", "0.15", FCVAR_USERINFO | FCVAR_DEMO, "Interpolate object positions starting this many seconds in past", true, 0.01, true, 1.0, cc_cl_interp_changed );  
//TGB: increased the following as it hopefully helps with the animation issues
static ConVar  cl_interp_npcs( "cl_interp_npcs", "0.25", FCVAR_USERINFO, "Interpolate NPC positions starting this many seconds in past (or cl_interp, if greater)", 0, 0, 0, 0, cc_cl_interp_changed );  
...
void C_BaseEntity::PreDataUpdate( DataUpdateType_t updateType )
{
	...
	//TGB: commenting the block below is listed in the wiki as jittery animation fix
	//	even though it doesn't work for us, it can't hurt to do so from what I gather
	//	apparently it at least fixes jittery player anims, which we also suffer from
	//TGB: does work when cl_interp_npc is used instead of clientside anim
	...
	//	//Msg("%s: Client set my animtime to %f\n", GetClassName(), m_flAnimTime);
	//}
