...
#include "c_hl2mp_player.h" //LAWYER:  Included for some obscure functions
...
extern void SpawnBlood(Vector vecSpot, const Vector &vecDir, int bloodColor, float flDamage); //qck edit
...
void C_ClientRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, char *pCustomImpactName )
{
	...
	SpawnBlood(pTrace->endpos, dir, BloodColor(), 100); // qck edit
	TraceBleed( 100, dir, pTrace, iDamageType ); // qck edit
...
bool C_BaseAnimating::DispatchMuzzleEffect( const char *options, bool isFirstPerson )
{
	...
	if ( token ) 
	{
		...
		//TGB: HACKHACK hexed the revolver to use this instead of SHOTGUN
		else if ( Q_stricmp( token, "DEFAULT" ) == 0 )
		{
			weaponType = MUZZLEFLASH_357;
		}
		else
...
C_BaseAnimating * C_BaseAnimating::BecomeRagdollOnClient( bool bCopyEntity )
{
	...
	if ( bCopyEntity )
	{
		C_HL2MPRagdoll *pRagdollCopy = new C_HL2MPRagdoll; //LAWYER:  ts2do fixed this, too.
...
//			pRagdollCopy->m_bFadeOut = true;  //LAWYER:  Something to do with crashes
		}
