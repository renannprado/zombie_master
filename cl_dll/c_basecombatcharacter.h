...
//TGB:
//-----------------------------------------------------------------------------
// Purpose: create a clientside effect that is manually updated and destroyed
//-----------------------------------------------------------------------------

class CFXCharSprite /*: public CClientSideEffect*/
{
public:
	...
	/*virtual*/ void	Update( Vector newpos, Vector newcolor, bool draw = true );
...
public:
	...
	//LAWYER:
	bool			m_bConqSelected;	//LAWYER:  Recieving end of the Selected function

private:
	//TGB:
	float				m_flHealthRatio; //TGB: receiving health from server
	CFXCharSprite		*m_HealthCircle;
	CFXCharSprite		*m_SelectCircle;

	void				CreateSelectCircle();
	void				CreateHealthCircle();
	virtual bool		ShouldHaveHealthCircle(){return false;}; //LAWYER:  Not everything has a health circle :P
...
public:
	...
	virtual int			DrawModel(int flags);  //LAWYER:  Overridden, BITCH!
	void				ZMSelectionEffects(); // qck edit
