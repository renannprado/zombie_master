...
#include <igameevents.h> //qck edit
...
//static ConVar zm_night_vision("zm_night_vision", "0", FCVAR_CHEAT);
...
//qck: Callback function to take care of zooming. 
//void __MsgFunc_ZoomBlur( bf_read &msg )
//{
//	rifleZoomed = msg.ReadOneBit();
//}

//USER_MESSAGE_REGISTER( ZoomBlur );
...
//Precache ZM "nightvision"
CLIENTEFFECT_MATERIAL( "effects/zm_nightvis" )
CLIENTEFFECT_MATERIAL( "postprocess/nightvision" )
CLIENTEFFECT_REGISTER_END()
//#endif
/*TGB: while looking at shader stuff because of a problem (that turned out to be a missing .dll)
	I noticed the CLIENTEFFECT_s above were within an ifdef, but below weren't. I corrected this
	for tidiness, with no resulting errors. Reverse it if it was supposed to be like that.*/

CLIENTEFFECT_REGISTER_BEGIN( PrecacheZoomOverlay )
CLIENTEFFECT_MATERIAL( "postprocess/blurx" )
CLIENTEFFECT_MATERIAL( "postprocess/blury" )
CLIENTEFFECT_MATERIAL( "postprocess/blend" )
CLIENTEFFECT_REGISTER_END()

CLIENTEFFECT_REGISTER_BEGIN( PrecacheGlow )
CLIENTEFFECT_MATERIAL( "models/red2" )
CLIENTEFFECT_MATERIAL( "models/orange" )
//CLIENTEFFECT_MATERIAL( "effects/zm_radius" )
CLIENTEFFECT_REGISTER_END()
#endif
...
void CViewRender::RenderViewEx( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	//qck: New check for this
	//if( rifleZoomed )

	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if (pPlayer)
	{
		if(pPlayer->m_bRifleBlur) 
			RifleZoomOverlay();
		if(pPlayer->m_bNightVis)
			ZM_NightVision();
	}
	else
		SetScreenOverlayMaterial( NULL );
...
void CViewRender::ZM_NightVision()
{
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if(pPlayer->IsZM())
	{
		...
		if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 81 )
		{	//shader
			pMaterial = materials->FindMaterial( "postprocess/nightvision", TEXTURE_GROUP_CLIENT_EFFECTS, true );
		}
		else
		{	//simple crappy thing
			pMaterial = materials->FindMaterial( "effects/zm_nightvis", TEXTURE_GROUP_OTHER, true );
		}

		if ( pMaterial )
		{
			UpdateScreenEffectTexture( 0, 0, 0, ScreenWidth(), ScreenHeight(), true ); //TGBMERGENOTE: extended with more arguments post-merge, bugpossibility
...
void CViewRender::RifleZoomOverlay()
{
	
	int w, h;

	
	ITexture *pTex0 = materials->FindTexture("_rt_blur_y", TEXTURE_GROUP_OTHER, true);
	ITexture *pTex1 = materials->FindTexture("_rt_blur_x", TEXTURE_GROUP_OTHER, true);
	...
	pMatBlurX = materials->FindMaterial( "postprocess/blurx", TEXTURE_GROUP_OTHER, true );
	if( IsErrorMaterial( pMatBlurX ) )
		return;
	pMatBlurY = materials->FindMaterial( "postprocess/blury", TEXTURE_GROUP_OTHER, true );
	if( IsErrorMaterial( pMatBlurY ) )
		return;
	pMatBlend = materials->FindMaterial( "postprocess/blend", TEXTURE_GROUP_OTHER, true );
	if( IsErrorMaterial( pMatBlend ) )
		return;
	...
	// Finally, blend and display
	materials->SetRenderTarget( pSaveRenderTarget );
	materials->Viewport( oldX, oldY, oldW, oldH );
	materials->DrawScreenSpaceQuad( pMatBlend );

}

//modified from code seen @ valve dev wiki by amckern
//TGB: simple nightvis start
static void ScreenOver_f( void )
{
	...
	//TGB: not sure why this would need to be static, but I'll leave it for now
	...
	{
		//TGB: don't load the material until we need it
		IMaterial *pMaterial = materials->FindMaterial( "postprocess/nightvision", TEXTURE_GROUP_OTHER, true );
		// turn it on
		view->SetScreenOverlayMaterial( pMaterial );
	...
	//TGB: I find this bit disturbing. Didn't we just turn it on? And now we turn it off? Whut?
...
static ConCommand r_screenover( "r_zm_nightvis", ScreenOver_f, "Nightvis", FCVAR_CHEAT );
