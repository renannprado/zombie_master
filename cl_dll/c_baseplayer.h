...
public:
	...
	bool					m_bHoldingObject; //qck: check for weapons
...
public:
	...
	//LAWYER STUFF
	int				m_iLastSelected; //Last selected entity's inx
	int				m_iLastCost; //Selected entity's cost
	int				m_iLastTrapCost; //Cost for trapping
	char     		m_szLastDescription[255]; //qck: Selected manip description (you know, we really might want to rewrite this so we don't have to pass through the player). 
//	virtual void ChangeTeam(int iTeamNum);
	int				m_iZombiePool; //LAWYER: The resources in the pool
	int				m_iZombiePopCount; //TGB: number of zombies in game
	int				m_iZombieSelected; //TGB: number of zombies selected

	int				m_iZombieGroup;    //qck: A handle to the group registry 
	int				m_iOldZombieGroup; //qck: Keeping track
	bool			m_bRoundRestart;   //qck: For client events

	int				m_iLastZombieFlags; //Lawyer:  Zombie Flags stuff

	char			m_ScreenEffect[256]; //qck: screen effect name

	void	UpdateScreenEffects(string_t screenEffect);
	
	//Shader Effect Stuff -- must find a cleaner way to do this 
	bool	m_bNightVis;
	bool	m_bRifleBlur;

	CUtlVector< int > m_ZombieGroupSerial;
	
	bool m_bCameraMode; //LAWYER: More shit.
protected:
