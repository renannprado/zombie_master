...
ConVar zm_fire_lighteffect("zm_fire_lighteffect", "1", FCVAR_ARCHIVE|FCVAR_CLIENTDLL, "Whether certain types of fire (molotov) are allowed to create performance-intensive lighting effects");
...
// C_FireSmoke
//==================================================
C_FireSmoke::C_FireSmoke(bool castlight)
{
	//Server-side
	m_flStartScale		= 0.0f;
	m_flScale			= 0.0f;
...
{
	//TGB: interpret the GLOW spawnflag as no light as well
	//can't do this in ctor because m_nFlags is 0 at that point
	if ((m_nFlags & bitsFIRESMOKE_GLOW) == false ||
		zm_fire_lighteffect.GetBool() == false) //if we never want it to glow
		m_bCastLight = false;
...
void C_EntityFlame::Simulate( void )
{
	...
	//TGB: we can have pretty fire too
//#ifdef HL2_EPISODIC 
...
void C_EntityFlame::AttachToHitBoxes( void )
{
	...
	for ( int i = 0; i < NUM_HITBOX_FIRES; i++ )
	{
		...
		m_pFireSmoke[i] = new C_FireSmoke(false); //TGB: firesmoke with no light effects
