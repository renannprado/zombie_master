...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_PlayerResource, DT_PlayerResource, CPlayerResource)
	...
	RecvPropArray3( RECVINFO_ARRAY(m_iZMPriority), RecvPropInt( RECVINFO(m_iZMPriority[0]))),
	RecvPropArray3( RECVINFO_ARRAY(m_iZMVoteRR), RecvPropInt( RECVINFO(m_iZMVoteRR[0]))),
	RecvPropArray3( RECVINFO_ARRAY(m_iZMImportance), RecvPropInt( RECVINFO(m_iZMImportance[0]))),
END_RECV_TABLE()
...
//	memset( m_iPacketloss, 0, sizeof( m_iPacketloss ) );
	...
	m_Colors[TEAM_ZOMBIEMASTER] = COLOR_GREEN;
...
int	C_PlayerResource::GetZMPriority( int iIndex )
{
	if ( !IsConnected( iIndex ) )
		return 0;

	return m_iZMPriority[iIndex];
}

int	C_PlayerResource::GetZMVoteRR( int iIndex )
{
	if ( !IsConnected( iIndex ) )
		return -1;

	return m_iZMVoteRR[iIndex];
}

int	C_PlayerResource::GetZMImportance( int iIndex )
{
	if ( !IsConnected( iIndex ) )
		return 0;

	return m_iZMImportance[iIndex];
}