...
void CFlashlightEffect::UpdateLightNew(const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp)
{
	...
	//TGB: flashlight power drain testing, effects are good
//#ifdef HL2_EPISODIC
	
	//C_BaseHLPlayer *pPlayer = (C_BaseHLPlayer *)C_BasePlayer::GetLocalPlayer();
	//if ( pPlayer && pPlayer->m_HL2Local.m_flSuitPower <= 10.0f )
	C_HL2MP_Player *pPlayer = C_HL2MP_Player::GetLocalHL2MPPlayer();
	if ( pPlayer && pPlayer->m_flFlashlightBattery <= 15.0f )
	{
		float flScale = SimpleSplineRemapVal(  pPlayer->m_flFlashlightBattery, 10.0f, 4.8f, 1.0f, 0.0f );
