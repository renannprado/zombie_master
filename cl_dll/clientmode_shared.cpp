...
void ClientModeShared::Init()
{
	...
	gameeventmanager->AddListener(this, "round_restart", false); //qck edit - This works again. Seems like a better place for it anyway. 
#ifndef _XBOX
	HLTVCamera()->Init();
#endif
...
int	ClientModeShared::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	{
		//switched wheelup and wheeldown around
		if ( down && keynum ==  K_MWHEELUP)
		{
			//DevMsg("Mousewheel down\n");
			
			engine->ClientCmd( "+movedown" );

			//TGB FIXME: this is hacky, getting it to -movedown after X milliseconds (think()-style) would be tidier
			for (int i=0;i<10;i++)
		...
		else if (down && keynum == K_MWHEELDOWN )
		{
			//DevMsg("Mousewheel up\n");
			engine->ClientCmd( "+moveup" );

			//TGB FIXME: this is hacky
			for (int i=0;i<10;i++)
				engine->ClientCmd( "wait" );
...
//--------------------------------------------------------------
// TGB: get at the actual chat element
//--------------------------------------------------------------
CBaseHudChat *ClientModeShared::GetChat()
{
	return m_pChatElement; //may be null
}
...
/*		if ( pTeam )
		...
		}*/ //LAWYER:  Silence the messages!
		if (team == 3)
		{ //LAWYER:  Alert people when there's a new ZombieMaster in town!
			hudChat->Printf( "Player %s has become the Zombie Master!\n", pPlayer->GetPlayerName());
		}
	...
	//qck: Clear decalse, etc. Any sort of cleanup on the client that needs to be done can be done in here.
	else if ( Q_strcmp( "round_restart", eventname) == 0)
	{
		hudChat->Printf( "Round complete. New round starting...\n");
		DevMsg("CLEARING DECALS\n"); //TGB: \n \n \n
		engine->ClientCmd( "r_cleardecals\n" ); //Clear Decals

		//LAWYER:  Clear props out.
		//FIXME:  This doesn't work properly.  We'll need to do it manually
		//C_BaseTempEntity::ClearDynamicTempEnts();
//		C_Gib

		//engine->ClientCmd( VarArgs( "zm_nightvision %i", false ));
		C_BasePlayer* pPlayer = C_BasePlayer::GetLocalPlayer(); 
		if (pPlayer)
			pPlayer->m_bRoundRestart = true;
	}
