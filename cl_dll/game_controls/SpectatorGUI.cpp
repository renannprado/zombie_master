...
void CSpectatorGUI::OnThink()
{
    ...
    if ( IsVisible() )
    {
		...
		//TGB: check target id and update if new
		if ( currenttarget != GetSpectatorTarget() )
			Update();
		else
		{
			IGameResources *gr = GameResources();
			if ( gr && currenthealth != gr->GetHealth( currenttarget ) )
				Update();
		}
    }
}
...
void CSpectatorGUI::ShowPanel(bool bShow)
{
	if ( bShow && !IsVisible() )
	{
		m_bSpecScoreboard = true; //LAWYER:  Spectator stuff!
	}
	SetVisible( bShow );
...
void CSpectatorGUI::Update()
{
	...
	if ( playernum > 0 && playernum <= gpGlobals->maxClients && gr )
	{
		...
		//TGB: save away target and health, to check if we need to update later
		currenttarget = playernum;
...
void CSpectatorGUI::UpdateScores()
{
    ...
    C_Team *teamElt = GetGlobalTeam(2);
    C_Team *teamRat = GetGlobalTeam(3);
