...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_RECV_TABLE_NOBASE(CPlayerState, DT_PlayerState)
		...
		RecvPropQAngles (RECVINFO(v_angle)),	//TGB: 0000069 spectator first person fix
		//RecvPropQAngles (RECVINFO(v_angle)), //qck: hlcoders first-person observer fix
	END_RECV_TABLE()
...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_RECV_TABLE_NOBASE( C_BasePlayer, DT_LocalPlayerExclusive )
		...
		RecvPropInt			( RECVINFO(m_iLastSelected)), //LAWYER: Transmit the last selected entity index
		RecvPropInt			( RECVINFO(m_iLastCost)), //LAWYER: Transmit the last selected entity index
		RecvPropInt			( RECVINFO(m_iLastTrapCost)), //LAWYER: Transmit the last selected entity index
		RecvPropInt			( RECVINFO(m_iLastZombieFlags)), //LAWYER: Used for Zombie Flags with entities.		
		RecvPropInt			( RECVINFO(m_iZombiePool)), //LAWYER: Transmit the last selected entity index
		RecvPropInt			( RECVINFO(m_iZombiePopCount)),
		RecvPropInt			( RECVINFO(m_iZombieSelected)),
		RecvPropInt			( RECVINFO(m_iZombieGroup)),
		RecvPropBool		( RECVINFO(m_bRoundRestart)), //qck edits
		RecvPropString		( RECVINFO(m_szLastDescription)), //qck edits a bit more
		RecvPropBool		( RECVINFO( m_bCameraMode )),
		RecvPropString		( RECVINFO( m_ScreenEffect )), //qck: screen space effects (shaders)
...
C_BasePlayer::C_BasePlayer() : m_iv_vecViewOffset( "C_BasePlayer::m_iv_vecViewOffset" )
{
	...
	m_iZombieGroup = NULL;
	m_iOldZombieGroup =	m_iZombieGroup;

	m_ZombieGroupSerial.Purge();
...
{
	//qck: Start off with all shader screen effects off
	m_bNightVis = 0;
	m_bRifleBlur = 0;
...
void C_BasePlayer::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	else
	{
		...
		//qck: If we have a new group handle, add it to the list. 
...
void C_BasePlayer::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if(m_iZombieGroup != m_iOldZombieGroup)
	{
		if((m_iZombieGroup != NULL))
		{
			m_ZombieGroupSerial.AddToTail(m_iZombieGroup);
			m_iOldZombieGroup = m_iZombieGroup;	
			DevMsg("Handles in list: %i\n", m_ZombieGroupSerial.Count());
		}
		
	}

	UpdateScreenEffects(m_ScreenEffect);

	//DevMsg("m_hZombieGroup: 0x%p\n", m_hZombieGroup.Get());
	//DevMsg("m_iOldZombieGroup: 0x%p\n", m_iOldZombieGroup.GetSerialNumber());
...
void C_BasePlayer::DetermineVguiInputMode( CUserCmd *pCmd )
{
	...
	if (m_pCurrentVguiScreen.Get() && (pCmd->buttons & (IN_ATTACK | IN_ATTACK2)) )
	{
		...
		pCmd->buttons &= ~(IN_ATTACK | IN_ATTACK2 | IN_VALIDVGUIINPUT); //LAWYER:  VGUI screen fix
		return;
	}
...
void C_BasePlayer::CreateMove( float flInputSampleTime, CUserCmd *pCmd )
{
	...
	// Check to see if we're in vgui input mode...
	if(pCmd->buttons & IN_VALIDVGUIINPUT) //LAWYER:  VGUI Screen fix
...
void C_BasePlayer::CalcChaseCamView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	...
	//TGB: account for an npc
	else if (target->IsNPC() && target->IsAlive())
	{
		VectorAdd( origin, VEC_VIEW, origin );
	}
	else
	...
	//TGB: 	0000422, view angles tilted while chasing zombie
	viewangles[ROLL] = 0;
...
void C_BasePlayer::CalcInEyeCamView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	...
	if ( !target ) 
	{
		...
		//TGB: argh, just zero roll here, hacky but at least it should work, and no sane person wants roll anyway

		//TGB: 0000389, view angles were not resetting after observed zombie died, specifically roll
		eyeAngles[ROLL] = 0;
		return;
	}
	...
	{
		bool result = target->GetAttachment( target->LookupAttachment( "eyes" ), eyeOrigin, eyeAngles );

		if (result == false)
		{
			//can't find an eye attachment, so revert to simple method
			eyeAngles = target->EyeAngles();
			eyeOrigin = target->GetAbsOrigin() + Vector(0, 0, 64); //hack for models with no eyes attachment, probably have no eyeposition either
		}
...
/*
void C_BasePlayer::ChangeTeam( int iTeamNum )
{
	//LAWYER:  Extra functionality, because I'm so damn good.
	m_iTeamNum = iTeamNum;
	if (iTeamNum == 3)
	{
		engine->ClientCmd("zombiepanel_open");
	}
	else
	{
		engine->ClientCmd("zombiepanel_close");
	}

}
*/

//qck: again, find a cleaner way to do this
void C_BasePlayer::UpdateScreenEffects(string_t screenEffect)
{
	if (Q_strcmp( screenEffect , "none" ) == 0 )
	{
		m_bRifleBlur = false;
		m_bNightVis = false;
	}
	else if (Q_strcmp( screenEffect , "blur" ) == 0 ) 
	{
		m_bRifleBlur = true;

	}
	else if (Q_strcmp( screenEffect , "nightvision" ) == 0 ) 
	{
		m_bNightVis = true;
	}
