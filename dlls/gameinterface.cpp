...
//qck edit
void CC_Version_Number( void )
{
	//TGB: is there a way we can get the compiler to dump in date/time here or something? 
	Msg("ZM Version Number: Beta 1.2.0\n"); // Update version number with each release
}

static ConCommand version_number("version_number", CC_Version_Number, "Displays the current version of the ZM dll's", FCVAR_CHEAT);
...
float CServerGameDLL::GetTickInterval( void ) const
{
	...
	//TGB: 
	//default rate is 66, ie. tickinterval of 0.015
	//CS:S increases interval to get a rate of 33, to reduce CPU
	//considering counterstrike, a much faster game, does it, I think we can safely do the same for ZM
	tickinterval *= 2;
...
{
	//LAWYER:  I owe this function to ts2do, from Wavelength.  He gets a beer someday.
	if(gpGlobals->maxClients>1)//You only need this test if you are making a single player portion of your mod with the hl2dm sdk
	{
		for(int i=gpGlobals->maxClients+1;i<gpGlobals->maxEntities;i++)
		{
			if ( CBaseCombatCharacter *pEntity = ToBaseCombatCharacter( UTIL_EntityByIndex( i ) ) )
			{
				//Remove all npcs from the game to prevent crashing
				pEntity->Remove();
			}
		}
	}
...
//LAWYER:  Crap to do with Maprules
//static ConVar motdfile( "motdfile", "motd.txt", 0, "The MOTD file to load.", ValidateMOTDFilename );
void CServerGameDLL::LoadMapInfo()
{
	char data[2048];
	char* mapname = UTIL_VarArgs( "maps\\%s.txt", STRING( gpGlobals->mapname ) );
//	Warning(mapname);

		int length = filesystem->Size( mapname, "GAME" );
		...
		FileHandle_t hFile = filesystem->Open( mapname, "rb", "GAME" );
		...
		g_pStringTableInfoPanel->AddString( "mapinfo", length+1, data );
...
void CServerGameClients::ClientSetupVisibility( edict_t *pViewEntity, edict_t *pClient, unsigned char *pvs, int pvssize )
{
	...
	//TGB: added ptr protection, testers got it to crash here. I know, I'm impressed as well.
	if (pPlayer)
	{
