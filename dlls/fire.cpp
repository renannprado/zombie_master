...
bool FireSystem_StartFire( const Vector &position, float fireHeight, float attack, float fuel, int flags, CBaseEntity *owner, fireType_e type )
{
	...
	fire->SetOwner( owner ); //tgb: set the owner BEFORE we spawn, so we can use it in the spawn func
	fire->Spawn();
...
//-----------------------------------------------------------------------------
// Purpose: LAWYER: Starts a fire at a specified location, with a crazy parent thing
// Input  : &position - position to start the fire at
//			flags - any special modifiers
//			DamageParent - a player that controls this
//-----------------------------------------------------------------------------
/* TGB: shouldn't be needed anymore

bool FireSystem_StartFireParented( const Vector &position, float fireHeight, float attack, float fuel, int flags, CBaseEntity *owner, fireType_e type, CBasePlayer *pDamageParent )
{	// TODO: if it turns out the owner is enough for damage attribution, remove this func [18-10-2008]
	VPROF_FIRE( "FireSystem_StartFire1" );
...
// 	if (pDamageParent)
// 	{
// 		fire->m_pDamageParent = pDamageParent;
// 	}
...
void CFire::Spawn( void )
{
	...
	//TGB: set this fire on the survivor team, if we are molotov-made
	CBaseEntity *pOwner = m_hOwner.Get();
	if (pOwner && pOwner->GetTeamNumber() == 2)
	{
		ChangeTeam(2);
	}
	...
	case FIRE_NATURAL:
		{
			...
			//fireSmoke->EnableVisibleFromAbove( ( m_spawnflags & SF_FIRE_VISIBLE_FROM_ABOVE )!=false ); //LAWYER:  Possible crash point?
			fireSmoke->EnableVisibleFromAbove( false );
...
void CFire::Update( float simTime )
{
	...
	{
		Warning("FIRE has owner\n");
		if (this->GetOwnerEntity()->IsPlayer())
		{
					Warning("FIRE Actually has a player owner\n");
		}
	}*/
	...
	for ( i = 0; i < nearbyCount; i++ )
	{
		...
		else if ( damage )
		{
			...
			if ( bDoDamage )
			{
				...
				if (tr.fraction == 1.0 && !tr.startsolid)
				{
					//TGB: I believe we do not need a separate damage parent, we can just use m_hOwner
					CBaseEntity *owner = m_hOwner.Get();
					//owner should be NULL if the player has disconnected or the entity died etc
					//TGB: make sure owner has not respawned as spectator, as that would mean he can damage survivors 0000432
					if (owner && (owner->GetTeamNumber() == 2 || friendlyfire.GetBool()) )
					{
						//DevMsg("Hurting %s with fire damage owned by %s\n", pOther->GetClassname(), owner->GetClassname());

						//making this stuff safe
						pOther->TakeDamage( CTakeDamageInfo( owner, owner, outputDamage, damageFlags ) );
					}
					//TGB: if we're molotov fire we're on the human team, and we shouldn't hurt humans unless ff is on
					else if ( pOther->GetTeamNumber() != this->GetTeamNumber() || friendlyfire.GetBool() )
					{
						//DevMsg("Hurting %s with fire damage owned by no one!\n", pOther->GetClassname());
						pOther->TakeDamage( CTakeDamageInfo( this, this, outputDamage, damageFlags ) );
					}
					

					/*
					if (m_pDamageParent) //LAWYER:  Parented fires need to deal maaagic damage!
					{
						//TGB: a molotov crash would happen here, due to m_pDamageParent referencing
						//	a player who had since disconnected.
						pOther->TakeDamage( CTakeDamageInfo( m_pDamageParent, m_pDamageParent, outputDamage, damageFlags ) );
					}
					else
					{
						pOther->TakeDamage( CTakeDamageInfo( this, this, outputDamage, damageFlags ) );
