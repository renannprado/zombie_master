...
//TGB
ConVar zm_stubborn_pathing("zm_stubborn_pathing", "1", FCVAR_NONE, "When 1, zombies will try harder to find a new path when they have been blocked. May affect performance.");
		...
		if ( fShouldMove )
		{
			...
			if ( result == AINR_OK )
			{
				...
				if ( IsMoveBlocked( moveResult ) )
				{
					...
					/*
						TGB: this is where zombies can get stuck by others of their kind being in the way
						oddly enough in these cases no attempt at rerouting is made. Let's give that a shot.
					*/

					if ( !bRecovered && zm_stubborn_pathing.GetBool() 
						&&  (moveResult == AIMR_BLOCKED_NPC || moveResult == AIMR_ILLEGAL)
						&& GetNavType() == NAV_GROUND )
					{
						//DevMsg("Move of (%i) blocked: %i. Attempting path refind... ", GetOuter()->entindex(), moveResult);
						AI_Waypoint_t *pSavedWaypoints = GetPath()->GetCurWaypoint();
						if ( pSavedWaypoints )
						{
							GetPath()->SetWaypoints( NULL );
							if ( RefindPathToGoal( false, true ) )
							{
								//DevMsg("Found new path!\n");
								DeleteAll( pSavedWaypoints );
								bRecovered = true;

								//TGB: prevent endless navigating
								Remember(bits_MEMORY_PATH_FAILED);
								if (m_timePathRebuildFail < gpGlobals->curtime)
									m_timePathRebuildFail = gpGlobals->curtime + 1.0f;
								
							}
							else
							{
								//DevMsg("No new path found.\n");
								GetPath()->SetWaypoints( pSavedWaypoints );
							}
