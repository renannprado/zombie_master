...
//TGB: added zm priority for use in ZM playerlist panel
...
//	SendPropArray( SendPropInt( SENDINFO_ARRAY(m_iPacketloss), 7, SPROP_UNSIGNED ), m_iPacketloss ),
	...
	//TGB
	SendPropArray3( SENDINFO_ARRAY3(m_iZMPriority), SendPropInt( SENDINFO_ARRAY(m_iZMPriority), 12 ) ),
	SendPropArray3( SENDINFO_ARRAY3(m_iZMVoteRR), SendPropInt( SENDINFO_ARRAY(m_iZMVoteRR), 12 ) ),
	SendPropArray3( SENDINFO_ARRAY3(m_iZMImportance), SendPropInt( SENDINFO_ARRAY(m_iZMImportance), 12 ) ),
END_SEND_TABLE()
...
void CPlayerResource::Spawn( void )
{
	for ( int i=0; i < MAX_PLAYERS+1; i++ )
	{
		...
		m_iTeam.Set( i, 0 );
		m_bAlive.Set( i, 0 );
		m_iZMPriority.Set( i, 0 );
		m_iZMImportance.Set( i, 0 );
	}
...
void CPlayerResource::UpdatePlayerData( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer && pPlayer->IsConnected() )
		{
			...
			m_iZMPriority.Set( i, pPlayer->GetZMPriority() );
			m_iZMVoteRR.Set( i, pPlayer->GetZMVoteRR() );
			m_iZMImportance.Set( i, pPlayer->GetZMImportance() );
