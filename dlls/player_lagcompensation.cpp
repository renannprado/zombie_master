...
ConVar sv_unlag( "sv_unlag", "1", 0, "Enables player AND NPC lag compensation" );
//TGB: default maxunlag was 1.0, we have lots of entities compensated so halved to 0.5, if your ping is more than that you're in trouble anyway
//UNDONE: reports of teleporting glitches, might be related
...
//TGB: addition for lagcomp tutorial from wiki
...
class CLagCompensationManager : public CAutoGameSystemPerFrame, public ILagCompensationManager
{
	...
	/*
	TGB: there are strong indications that the memleak occurs due to lag compensation, with sv_unlag
		effectively turning off the leak. This is not desirably obv., so attempts are being made to
		fix things.

		One obvious way to simplify the structure was taking out the AI index based indexing of the
		NPCs. It required reshuffling every tick if an NPC had died, which is messy and error-prone.
		An attempt was made to use entindices or EHANDLEs instead using UtlMaps, but this ended up
		having some crash during the precache phase (where a few npcs are created and immediately
		destroyed) with useless stack traces.

		As a simpler solution, the NPC-specific data was then moved into the NPC class itself, 
		making indexing issues go away, and reducing manual memory management.
	*/
...
public:
	CLagCompensationManager( char const *name ) : CAutoGameSystemPerFrame( name )
	{
		...
		/*
		//TGB: the below is all kind of useless seeing as we have to Purge when an NPC is removed
		//	as RemoveAll crashes, for unknown reasons. Meaning we need to realloc anyway.
		if (sv_unlag.GetBool())
		{
			//TGB: try to reserve a big chunk of memory for our goods so we need fewer alloc/dealloc crap
			//tickrate = 33, default unlag = 0.5 secs, makes 16.5 saved timepoints, adding a bit as buffer
			const int size = ceil(33 * sv_maxunlag.GetFloat()) + 5;
			for ( int i=0; i<MAX_PLAYERS; i++ ) // all players
			{
				m_PlayerTrack[i].Purge();
				m_PlayerTrack[i].EnsureCapacity(size);
			}
			//for ( int j=0; j<MAX_AIS; j++ ) // and all npcs
			//{
			//	m_EntityTrack[i].Purge();
			//	m_EntityTrack[j].EnsureCapacity(size);
			//}
		}
		*/

		/*
		m_EntityTrack.AccessTree()->Purge();
		m_RestoreEntity.AccessTree()->Purge();
		m_EntityRestoreData.AccessTree()->Purge();
		m_EntityChangeData.AccessTree()->Purge();
		*/

		DevMsg("LagComp manager created\n");
	}
	...
	void			RemoveNpcData(EHANDLE handle) // clear specific NPC's history
	{
		//CUtlFixedLinkedList< LagRecord > *track = &m_EntityTrack[index];
		//track->Purge();

		//TGB: this is simpler:
		//m_EntityTrack[index].Purge();
		/*
		//clear track
		int idx = m_EntityTrack.Find(handle);
		if (m_EntityTrack[idx])
		{
			//m_EntityTrack[index]->Purge();
			delete m_EntityTrack[idx];
		}
		m_EntityTrack.Remove(handle);

		//clear restore flag
		m_RestoreEntity.Remove(handle);

		//clear restoredata
		m_EntityRestoreData.Remove(handle);

		//clear changedata
		m_EntityChangeData.Remove(handle);
		*/
	}

	void			AddNpcData(EHANDLE handle)
	{
		/*
		//prepare a track
		//CUtlFixedLinkedList<LagRecord> *track = new CUtlFixedLinkedList<LagRecord>();
		const int size = ceil(33 * sv_maxunlag.GetFloat()) + 5;
		//CUtlLinkedList<LagRecord> *track = new CUtlLinkedList<LagRecord>(10, size);
		//m_EntityTrack.Insert(index, track);
		int idx = m_EntityTrack.Insert(handle);

		m_EntityTrack[idx] = new CUtlLinkedList<LagRecord>(10, size);
        m_EntityTrack[idx]->Purge();
		//m_EntityTrack[index]->EnsureCapacity(size);

		//prepare restore flag
		m_RestoreEntity.Insert(handle, false);

		//prepare restoredata
		m_EntityRestoreData.Insert(handle);

		//prepare changedata
		m_EntityChangeData.Insert(handle);
		*/
	}

	//TGB: moved into public so we can force it on roundrestart
	void ClearHistory()
	{
		DevMsg("CLagCompensationManager: clearing history\n");
		for ( int i=0; i<MAX_PLAYERS; i++ ) // all players
			m_PlayerTrack[i].Purge();

		//TGB: with lagcomp data saved in NPC, clearing it has very little use here
		//	whenever this func is called, all NPCs are destructed or about to be

		//for ( int j=0; j<MAX_AIS; j++ ) // and all npcs
		//	m_EntityTrack[j].Purge();

		//below should already be clean, but what the heck
		/*
		for ( int i = m_EntityTrack.FirstInorder(); i != m_EntityTrack.InvalidIndex(); i = m_EntityTrack.NextInorder( i ) )
		{
			if (m_EntityTrack[i] != NULL)
				delete m_EntityTrack[i];
		}

		m_EntityTrack.RemoveAll();

		m_RestoreEntity.RemoveAll();
		m_EntityRestoreData.RemoveAll();
		m_EntityChangeData.RemoveAll();
		*/
	}
...
private:
	...
	//TGB: experiment: switched this crap to an CUtlMap for the NPCs so we can avoid shuffling indices
...
/*
	...
	CUtlMap<EHANDLE, CUtlLinkedList<LagRecord>* > m_EntityTrack;

	//CBitVec<MAX_AIS>	m_RestoreEntity;
	CUtlMap<EHANDLE, bool > m_RestoreEntity;
	
	//LagRecord		m_EntityRestoreData[ MAX_AIS ];
	//LagRecord		m_EntityChangeData[ MAX_AIS ];

	CUtlMap<EHANDLE, LagRecord> m_EntityRestoreData;
	CUtlMap<EHANDLE, LagRecord> m_EntityChangeData;
*/
...
//		m_bNeedsAIUpdate = true;
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		...
		//CUtlFixedLinkedList< LagRecord > *track = &m_EntityTrack[eidx];
		//CUtlFixedLinkedList< LagRecord > *track = m_EntityTrack[eidx];

		//TGB: utlmap lagcomp
//		EHANDLE handle = pNPC;
//		int trackidx = m_EntityTrack.Find(handle);
//		CUtlLinkedList< LagRecord > *track = m_EntityTrack[trackidx];
		...
		//	track->RemoveAll();
		//	//track->Purge();
		//	//TGB: purging has not shown to solve the memleak, so undoing that
		//	continue;
		//}

		Assert( track->Count() < 1000 ); // insanity check

		//TGB: asserts won't help us
		if (track->Count() > 1000)
		{
			Warning("LagCompensation: tracklength over 1000 (%i) for ent %s with track %i\n", track->Count(), pNPC->entindex(), i);
		//	DebuggerBreak();
		}
...
void CLagCompensationManager::StartLagCompensation( CBasePlayer *player, CUserCmd *cmd )
{
	...
	//TGB: need to set all flags to false
	//for ( int i = m_RestoreEntity.FirstInorder(); i != m_RestoreEntity.InvalidIndex(); i = m_RestoreEntity.NextInorder( i ) )
	//	m_RestoreEntity[i] = false;
	...
	if ( !player->m_bLagCompensation		// Player not wanting lag compensation
		 ...
		 || player->IsZM()					//TGB: ZM moves like a spectator
		)
		return;
...
/* utlmap lagcomp
	//TGB: none of this array memset crap is possible now
	for ( int i = m_EntityRestoreData.FirstInorder(); i != m_EntityRestoreData.InvalidIndex(); i = m_EntityRestoreData.NextInorder( i ) )
		m_EntityRestoreData[i] = LagRecord();

	for ( int i = m_EntityChangeData.FirstInorder(); i != m_EntityChangeData.InvalidIndex(); i = m_EntityChangeData.NextInorder( i ) )
		m_EntityChangeData[i] = LagRecord();
*/
	//TGB: lagcomp in NPC means it's handier to clear on case by case basis
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		...
		//TGB: lagcomp in npc: clear restore/change data before we proceed
		pNPC->m_RestoreData = LagRecord();
		pNPC->m_ChangeData = LagRecord();
			...
			else
			{
				...
				if ( pHitEntity )
				{
					...
					for ( int i = 0; i < nAIs; i++ ) // we'll have to find this entity's index though :(
					{
						if ( pNPC == pHitEntity )
						{
							//TGB: moved the assignment into the clause, else pNPC could have
							//	a non-NULL value even though hitentity was not found for some reason
							pNPC = ppAIs[i];
							break;
						}
					}*/

					CAI_BaseNPC *pNPC = pHitEntity;

					/* utlmap lagcomp
					// If we haven't backtracked this player, do it now
					// this deliberately ignores WantsLagCompensationOnEntity.
					const EHANDLE handle = pNPC;
					const int idx = m_RestoreEntity.Find(handle);
					const bool torestore = m_RestoreEntity[idx];
					//if ( pNPC && !m_RestoreEntity.Get( pNPC->GetAIIndex() ) )
					if ( idx != INVALID_EHANDLE_INDEX && torestore == false )
					{
						...
						m_RestoreEntity[idx] = true;

						BacktrackEntity( pHitEntity, flTargetTime );

						// Remove the temp flag
						//m_RestoreEntity.Clear( pNPC->GetAIIndex() );
						m_RestoreEntity[idx] = false;
					}
					*/

					//TGB: lagcomp in npc
					if (pNPC && pNPC->m_bRestoringNPC != true)
					{
						pNPC->m_bRestoringNPC = true;

						BacktrackEntity( pNPC, flTargetTime );

						pNPC->m_bRestoringNPC = false;
					}
				}
			}
...
*/
	...
	/* utlmap lagcomp
	const int restore_idx = m_EntityRestoreData.Find(handle);
	LagRecord *restore = &m_EntityRestoreData[ restore_idx ];
	const int change_idx = m_EntityRestoreData.Find(handle);
	LagRecord *change  = &m_EntityChangeData[ change_idx ];
	*/

	LagRecord *restore = &pEntity->m_RestoreData;
	LagRecord *change = &pEntity->m_ChangeData;
	...
	/* utlmap lagcomp
	int ridx = m_RestoreEntity.Find(handle);
	m_RestoreEntity[ridx] = true;
	*/

	//lagcomp in npc
	pEntity->m_bRestoringNPC = true;
...
void CLagCompensationManager::FinishLagCompensation( CBasePlayer *player )
{
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		...
		/* utlmap lagcomp
		const EHANDLE handle = pNPC;
        const int ridx = m_RestoreEntity.Find(handle);
		if ( !m_RestoreEntity[ridx] )
		{
		...
		const int restore_idx = m_EntityRestoreData.Find(handle);
		LagRecord *restore = &m_EntityRestoreData[ restore_idx ];
		const int change_idx = m_EntityChangeData.Find(handle);
		LagRecord *change  = &m_EntityChangeData[ change_idx ];
		*/

		if (pNPC->m_bRestoringNPC != true)
			continue;
