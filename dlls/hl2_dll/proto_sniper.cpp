...
//qck: Override the default relationship with a special one making an AI hate other AI's. 
Disposition_t CProtoSniper::IRelationType ( CBaseEntity *pTarget )
{
	if ( pTarget )
	{
		//if(Q_strcmp(pTarget->GetClassname(), "C_AI_BaseNPC") == 0) 
		if( pTarget->IsNPC() )
		{
			return D_HT; 
...
void CProtoSniper::InputSweepGroupRandomly( inputdata_t &inputdata )
{
	...
	//qck: Hmmz, do we really want to use something that Valve themselves calls a performance hog? -qck
...
int CProtoSniper::RangeAttack1Conditions ( float flDot, float flDist )
{
	...
	{
		//DevMsg("Sniper has enemy of class %s\n", GetEnemy()->GetClassname() );
...
Vector CProtoSniper::DesiredBodyTarget( CBaseEntity *pTarget )
{
	...
	else
	{
		...
		else if( pTarget->Classify() == CLASS_ZOMBIE )
		{
			...
			//qck: Shoot zombies in the heart
