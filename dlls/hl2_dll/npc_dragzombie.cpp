...
class CNPC_DragZombie : public CAI_BlendingHost<CNPC_BaseZombie>
{
	DECLARE_CLASS( CNPC_DragZombie, CAI_BlendingHost<CNPC_BaseZombie> );
...
//	virtual int RangeAttack2Conditions( float flDot, float flDist );
	...
	int GetPopCost() { return zm_popcost_drifter.GetInt(); }

	int	GetZombieType() { return TYPE_DRIFTER; }
...
//	PrecacheScriptSound( "NPC_DragZombie.ThrowWarn" );
//	PrecacheScriptSound( "NPC_DragZombie.Throw" );
...
// Purpose: 
//-----------------------------------------------------------------------------
Class_T	CNPC_DragZombie::Classify( void )
{
	return CLASS_ZOMBIE;
}
...
//-----------------------------------------------------------------------------
bool CNPC_DragZombie::MustCloseToAttack(void)
{
	return true;
}
...
//			come through.
//-----------------------------------------------------------------------------
/*void CNPC_DragZombie::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	if( !m_pMoanSound )
	{
	...
	envDragZombieMoanVolumeFast[ 1 ].durationMin = 0.1;
	envDragZombieMoanVolumeFast[ 1 ].durationMax = 0.4;
...
//-----------------------------------------------------------------------------
bool CNPC_DragZombie::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	return false;
}
