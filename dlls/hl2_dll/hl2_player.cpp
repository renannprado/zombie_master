...
static ConCommand toggle_zoom("toggle_zoom", CC_ToggleZoom, "Toggles zoom display", FCVAR_CHEAT ); //cheatified
...
#define SUITPOWER_CHARGE_RATE 0.001 //TGB: recharge should be disabled, but just in case
...
void CHL2_Player::EquipSuit( bool bPlayEffects )
{
	...
	{
		//StartAdmireGlovesAnimation(); //LAWYER:  No glove admiration
	}
}
...
//					Warning( "In train mode with no train!\n" );
	...
	//TGB: moved in from baseplayer, as baseclass::prethink is not called here
...
/*	if ( interactionType == g_interactionBarnacleVictimDangle )
	...
	else if (interactionType ==	g_interactionBarnacleVictimGrab)
	{
		...
		ClearUseEntity();
		return true;
	}*/ //LAWYER:  We don't actually have barnacles.
	return false;
}
...
{
	//TGB: cheats were allowed without sv_cheats
...
void CHL2_Player::SuitPower_Update( void )
{
	...
	else if( m_HL2Local.m_bitsActiveDevices )
	{
		...
		if( !SuitPower_Drain( flPowerLoad * gpGlobals->frametime ) )
		{
			...
			if( FlashlightIsOn() )
			{
				//TGB: let's try re-enabling flashlight power outage for kicks
//#ifndef HL2MP
				FlashlightTurnOff();
//#endif
			}
...
bool CHL2_Player::SuitPower_RemoveDevice( const CSuitPowerDevice &device )
{
	...
	//TGB: UNDONE: if we have finite flashlight power we don't want random drains like this, and we have no sprint anyway
...
bool CHL2_Player::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	...
	//	return false; //LAWYER:  We can ALWAYS switch to ammoless weapons, for throwing.
...
void CHL2_Player::ExitLadder()
{
	...
	// 0000101 TGB: make sure we unholster weapon
	if ( GetActiveWeapon() )
