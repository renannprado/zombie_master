...
//
// None of the above is true for ZM zombies.
//
//=============================================================================//
...
ConVar zm_popcost_banshee("zm_popcost_banshee", "4", FCVAR_NOTIFY | FCVAR_REPLICATED, "Population points taken up by banshees");
ConVar zm_popcost_hulk("zm_popcost_hulk", "3", FCVAR_NOTIFY | FCVAR_REPLICATED, "Population points taken up by hulks");
ConVar zm_popcost_shambler("zm_popcost_shambler", "1", FCVAR_NOTIFY | FCVAR_REPLICATED, "Population points taken up by shamblers");
ConVar zm_popcost_immolator("zm_popcost_immolator", "5", FCVAR_NOTIFY | FCVAR_REPLICATED, "Population points taken up by immolators");
ConVar zm_popcost_drifter("zm_popcost_drifter", "2", FCVAR_NOTIFY | FCVAR_REPLICATED, "Population points taken up by drifters");
...
#define ZOMBIE_PHYSICS_SEARCH_DEPTH	100 //was: 200, TGB: changed because high values increase complexity by quite a bit, and we waste loads of cycles on AI already

// Don't swat objects unless player is closer than this.
#define ZOMBIE_PLAYER_MAX_SWAT_DIST		700 //was: 2000, TGB: that was rather far, and we're reducing vision range anyway
...
#define ZOMBIE_MAX_PHYSOBJ_MASS		200 //TGB: def 60. Zombies are persistent chaps, they can pound away at bigger stuff
...
ConVar	sk_zombie_dmg_one_slash( "sk_zombie_dmg_one_slash","25");
ConVar	sk_zombie_dmg_both_slash( "sk_zombie_dmg_both_slash","50");
...
//ConVar zm_defense("zm_defense", 0);

ConVar zm_zombieswatforcemin("zm_zombieswatforcemin", "20000");
ConVar zm_zombieswatforcemax("zm_zombieswatforcemax", "70000");
ConVar zm_zombieswatlift("zm_zombieswatlift", "20000");
...
CNPC_BaseZombie::CNPC_BaseZombie()
{
	...
	m_bIsInAmbush = false;
	m_bSwarmAmbushPoint = false;
	m_pAmbushPoint = NULL;

	m_fFadeFinish = 0;

	//TGB: add zombie to total zombielist
...
CNPC_BaseZombie::~CNPC_BaseZombie()
{
	...
	//LAWYER:  This would be a good place to strip them from groups, too
	...
	if (pManEnt)
	{
		...
		if( pManager )
		{
			pManager->DropZombieFromAll(this);
...
bool CNPC_BaseZombie::FindNearestPhysicsObject( int iMaxMass, int iMaxDistance )
{
	...
	{
		//TGB: no enemy, so simply check if there's any physics crap between us and the move goal
		vecTargetPos = GetNavigator()->GetGoalPos();
		//DevMsg("GetGoalPos: %f %f %f vs AbsOrig: %f %f %f\n", vecTargetPos.x, vecTargetPos.y, vecTargetPos.z, GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z);
	}
	else
		vecTargetPos = GetEnemy()->GetAbsOrigin();

	//TGB: replaced all GetEnemy()->GetAbsOrigin() calls with vecTargetPos
	
	//TGB: parameter for max object distance
	...
	if( dist > ZOMBIE_PLAYER_MAX_SWAT_DIST )
	{
		...
		// they are closer.
		//DevMsg("Not finding object due to distance\n");
		return false;
	}
	...
	//TGB: build a debug line
	//DevMsg("It:\n");
	for( i = 0 ; i < count ; i++ )
	{
		//DevMsg("%i: %s, ", i, pList[i]->GetClassname());
		...
		{
			//DevMsg("closer to target than to me.\n");
			continue;
		}
		...
		// Don't swat server ragdolls! TGB: oh snap
...
float CNPC_BaseZombie::MaxYawSpeed( void )
{
	...
	{
		return( 20 ); //TGB: slightly increased from 15
	}
	else
	{
		switch( GetActivity() )
		...
		case ACT_RUN:
			...
			break;
		case ACT_WALK:
			return 25; //TGB: detach walk and idle
		case ACT_IDLE:
			return 35; //TGB: we can turn faster when we're standing still
			break;
...
void CNPC_BaseZombie::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	//DevMsg("Zombie hit: HG: %i, HB: %i, CLB: %b\n", ptr->hitgroup, ptr->hitbox, (infoCopy.GetDamageType() & DMG_CLUB));

	// Headshots kill pretty quickly, even with a pistol
	if (ptr->hitgroup == HITGROUP_HEAD)
	{
		//TGB: getting hit in the head with crushing damage really hurts a zombie's poor brains
		if ( infoCopy.GetDamageType() & DMG_CLUB )
		{
			//TGB: sometimes you just don't hit them right, so semirandom scaling (also to avoid overpowering)
			infoCopy.ScaleDamage( random->RandomFloat( 1.90, 3.00) ); 
			
		}
		else
		{
			//other damage types also scale up
			infoCopy.ScaleDamage( 2.10 ); 
		}

		UTIL_ShouldShowBlood(BLOOD_COLOR_RED);
		UTIL_BloodImpact(ptr->endpos, vecDir, BLOOD_COLOR_RED, 100);
	}
	...
	//DevMsg("Zombie got hit for %f so far\n", infoCopy.GetDamage() );
...
{
	return false; //LAWYER:  Hack, but I have a sneaking suspicion this is causing crashes.
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	//fire zombie_hurt
	HL2MPRules()->ZombieHurt(this, info);
...
{
	//TGB: never, ever, EVER die chopped. We don't have gib models.
	return;
...
bool CNPC_BaseZombie::ShouldIgnite( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_BURN )
	{
		...
		//TGB: in ZM, we want molotovs to burn up zombies that pass through fairly easily
		//	so, increased interval to 6 seconds, and reduced requirement to 5%

		m_flBurnDamage += info.GetDamage();
		m_flBurnDamageResetTime = gpGlobals->curtime + 6;

		if ( m_flBurnDamage >= m_iMaxHealth * 0.05 )
		{
			return true;
		}
	}
...
void CNPC_BaseZombie::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
	...
	//TGB: none of our zombies have this
...
{
	//LAWYER:  Zombies do fire damage when on fire, and deal extra whammy.
	int iDamageType = DMG_SLASH; //The type of damage the Zombie will inflict.

	if (this->IsOnFire())
	{
		iDamage = iDamage * (random->RandomFloat(1,2)); //LAWYER:  Random amount of bonus
		iDamageType = DMG_BURN; //TGB: this was slash, but should be burn. We default to slash
	}
	...
	if ( !pHurt && m_hPhysicsEnt != NULL && IsCurSchedule(SCHED_ZOMBIE_ATTACKITEM) )
	{
		...
		CTakeDamageInfo info( this, this, vForce, GetAbsOrigin(), iDamage, iDamageType );
	...
	if ( pHurt )
	{
		...
		if ( pPlayer != NULL && !(pPlayer->GetFlags() & FL_GODMODE ) )
		{
			...
			//TGB: this basically does nothing. This entire clawattack function is madness.
			//		Can you tell where the damage is dealt, for example? Go on, it's like a quiz.
			//		Spoilers: It's not here, it's not in checktracehullattack, and it's not in
			//		the function that called this.
			//Older and wiser TGB: actually it IS in checktracehullattack, surprise!
			pPlayer->VelocityPunch( vecVelocityPunch );

			
			//TGB: draw blood impact decal
			const float flNoise = 6.0f;
			trace_t traceHit;

			//all zombies dish out the pain, so draw a bunch of bloodspots
			//a single one can be hard to see
			for (int i = 0; i < 6; i++)
			{
				Vector vecTraceDir;
				...
				{
					vecTraceDir = pPlayer->EyePosition(); //once in a while we go for the eyes
					vecTraceDir.z -= random->RandomFloat( -flNoise, 0.0f );
				}
				else
				{
/*					vecTraceDir = pHurt->GetAbsOrigin();
					//origin will be at feet, so shove it upwards by some amount
					vecTraceDir.z += random->RandomFloat( 10, 70); //may overshoot while crouching

					//add tad of variance
					vecTraceDir.x += random->RandomFloat( -flNoise, flNoise );
					vecTraceDir.y += random->RandomFloat( -flNoise, flNoise );
*/
					//TGB: maybe if we just trace forward a ways from our position we'll hit more

					//dark vector magic I only half understand, but valve does it
					...
					vecTraceDir = WorldSpaceCenter() + (forward * 128 );

					vecTraceDir.x += random->RandomFloat( -flNoise, flNoise );
					vecTraceDir.y += random->RandomFloat( -flNoise, flNoise );
					vecTraceDir.z += random->RandomFloat( -flNoise, flNoise + 10.0f );
				}
				//vecTraceDir.z += random->RandomFloat( -flNoise, flNoise );


				UTIL_TraceLine( WorldSpaceCenter(), vecTraceDir, MASK_SHOT_HULL, this, COLLISION_GROUP_NONE, &traceHit );

				DoImpactEffect( traceHit, DMG_BULLET );

//				if (traceHit.DidHitNonWorldEntity())
//					DevMsg("Zombie bloodtrace hit %s\n", traceHit.m_pEnt->GetClassname());
			}
		...
		/*else if( !pPlayer && UTIL_ShouldShowBlood(pHurt->BloodColor()) )*/
		{
			...
			/* TGB: most of our models have none of these
			switch( BloodOrigin )
			{
	...
	if ( m_bSwatBreakable || pHurt == m_hPhysicsEnt /*&& IsCurSchedule(SCHED_ZOMBIE_ATTACKITEM)*/ ) //removed second part to make sure we reset
	{
		//DevMsg("Resetting swatstuff\n");
		m_bSwatBreakable = false;
		m_hPhysicsEnt = NULL;
...
///--------------------------------------------------------------
// TGB: perform swatting of an object
//--------------------------------------------------------------
void CNPC_BaseZombie::SwatObject( IPhysicsObject *pPhysObj, Vector direction )
{
	const float targetmass = pPhysObj->GetMass();

	//give the object some upwards force as we do want a bit of a throw esp. for light objects
	const float liftforce = RemapVal( targetmass, 5, 350, 3000, zm_zombieswatlift.GetFloat() );
	Vector uplift = Vector(0, 0, liftforce);

	//determine the forward force
	const float swatforce = RemapVal( targetmass, 5, 500, zm_zombieswatforcemin.GetFloat(), zm_zombieswatforcemax.GetFloat() );
	DevMsg("Zombie swatting object of mass %f with force %f\n", targetmass, swatforce);
...
//			MoanSound( envDefaultZombieMoanVolumeFast, ARRAYSIZE( envDefaultZombieMoanVolumeFast ) );
	...
	if ( pEvent->event == AE_ZOMBIE_SWATITEM )
	{
		...
		if ( pEnemy )
		{
			...
			//TGB: turned this from a velocity override into a physics push...
		...
		{
			//TGB: we're attacking a physobj but not to throw it at someone
			...
			//TGB: changed up to fit new swatting style and method
			//get forward dir
			Vector forward;
			...
			//we do this not unlike the physcannon/gravgun
			/*float flForce = 1000;
			...
			{
				mass = min(mass, 1000);
				flForce = SimpleSplineRemapVal(mass, 100, 600, 1000, 500);
			}
			
            //determine force and angles
			Vector vVel = forward * flForce;
			AngularImpulse aVel( random->RandomFloat(-180, 180), 20, random->RandomFloat(-360, 360) );
				
			//apply
			pPhysObj->AddVelocity( &vVel, &aVel );
			*/
...
void CNPC_BaseZombie::Spawn( void )
{
	...
	SetZombieMode( ZOMBIE_MODE_OFFENSE ); //qck: Start on offense (normal AI).

	//TGB: don't stand by me... I mean, ON me
	//nevermind, this is already done in NPCInit
	...
	//TGB: lookdistance is normally 2048. Lower that stuff holmes
	//this has to be after NPCInit btw, 'cause that one forces it to 2048
	GetSenses()->SetDistLook(1024);

	m_vecLastGoalPosition = GetAbsOrigin(); //qck: Try this out...


	//TGB: have the zombie register itself at the ZM
	//	wouldn't hurt to have this happen elsewhere, but this guarantees we get all spawned zombies
...
//qck: Called to change modes. Offense, defense, etc.
bool CNPC_BaseZombie::SetZombieMode(Zombie_Modes zomMode)
{
	
	m_zombieMode = zomMode;
	return true;
}
...
//	UTIL_PrecacheOther( GetHeadcrabClassname() );
	...
	UTIL_PrecacheOther( "info_ambush_point" ); //TGB: to avoid late precaching
...
void CNPC_BaseZombie::BuildScheduleTestBits( void )
{
	...
	//qck: If we're chasing, and we must return to our position, do so ASAP.
	if( IsCurSchedule(SCHED_ZOMBIE_CHASE_ENEMY || SCHED_ZOMBIE_SWATITEM))
	{
		SetCustomInterruptCondition( COND_ZOMBIE_RETURN_TO_POSITION );	
	}

	if( IsCurSchedule(SCHED_ZOMBIE_RETURN_TO_POSITION))
	{
		SetCustomInterruptCondition( COND_LIGHT_DAMAGE );
		SetCustomInterruptCondition( COND_ZOMBIE_DEFENDING );
	}
...
*/
	...
	//TGB: process cancellation of swat order when getting new orders
	if ( HasCondition( COND_RECEIVED_ORDERS ) )
	{
		//no need to do anything other than clear this condition unless we have swat stuff going on
		if (m_hPhysicsEnt != NULL || HasCondition( COND_ZOMBIE_CAN_SWAT_ATTACK))
		{
			m_hPhysicsEnt = NULL;
			m_bSwatBreakable = false;
	...
	//qck: If attacking, make sure to return to defending after you've chased the enemy away
	if ( HasCondition( COND_ZOMBIE_RETURN_TO_POSITION ) )
		return SCHED_ZOMBIE_DEFEND_POSITION;

	if( HasCondition( COND_ZOMBIE_DEFENDING ))
	{
		return SCHED_ZOMBIE_DEFEND_POSITION;
	}
	if( HasCondition( COND_ZOMBIE_IN_AMBUSH ))
	{
		return SCHED_ZOMBIE_AMBUSH_MODE;
	}
	...
	case NPC_STATE_COMBAT:
		...
		{
			return SCHED_ZOMBIE_WANDER_SHORT; //TGB: was _MEDIUM
		}
	...
	case NPC_STATE_ALERT:
		...
		break;
		//TGB: added for swatting while just moving around
	case NPC_STATE_IDLE:
		if( HasCondition( COND_ZOMBIE_CAN_SWAT_ATTACK ) )
		{
			//DevMsg("Found swat condition in idle\n");
			return SCHED_ZOMBIE_SWATITEM;
		}
		//qck: If the zm toggles defense, go to special defense schedule
		else if(m_zombieMode == ZOMBIE_MODE_DEFENSE)
		{ 
			return SCHED_ZOMBIE_DEFEND_POSITION;
		}
		else if(m_zombieMode == ZOMBIE_MODE_AMBUSH)
		{
			return SCHED_ZOMBIE_AMBUSH_MODE;
		}

		if( HasCondition(COND_ZOMBIE_RETURNING))
			return SCHED_ZOMBIE_RETURN_TO_POSITION;
...
int CNPC_BaseZombie::GetSwatActivity( void )
{
	...
	//TGB: make sure we have something that will work
	if (m_bSwatBreakable || !CanSwatPhysicsObjects())
	{
		DevMsg("Attacking object with melee\n");
...
void CNPC_BaseZombie::GatherConditions( void )
{
	...
	if( m_NPCState == NPC_STATE_COMBAT && !m_fIsTorso )
	{
		...
		{
			//TGB: initially I figured we could just make zombies check for physobj while walking
			//however, if a route registers as blocked to the pathfinder this does not suffice
			//instead, we need to find a way to let the ZM target a specific object, which the zombie then bashes forward

			//DevMsg("Finding a physics object to swat in state %i\n", m_NPCState);
			//if ( m_NPCState == NPC_STATE_COMBAT )
				FindNearestPhysicsObject( ZOMBIE_MAX_PHYSOBJ_MASS );
			//else
			//	FindNearestPhysicsObject( ZOMBIE_MAX_PHYSOBJ_MASS, 100 ); //limited range if not in combat

			m_flNextSwatScan = gpGlobals->curtime + 2.0;
		}
	}
	
	//if (m_hPhysicsEnt != NULL) DevMsg("PHYSOBJ FOUND\n");

	if( (m_hPhysicsEnt != NULL) && gpGlobals->curtime >= m_flNextSwat /*&& HasCondition( COND_SEE_ENEMY )*/ )
	{
		//DevMsg("Setting swat condition\n");
	...
	if(m_zombieMode == ZOMBIE_MODE_DEFENSE ) //qck: Set defense flag if needed
	{
		SetCondition(COND_ZOMBIE_DEFENSE_MODE);
	}

	if(m_zombieMode == ZOMBIE_MODE_AMBUSH )
	{
		SetCondition( COND_ZOMBIE_IN_AMBUSH );
	}

	
	//qck: Cripes, this is ugly. 
	if(HasCondition(COND_ZOMBIE_DEFENSE_MODE))
	{
		if(GetEnemy())
		{
			//float dist = EnemyDistance( GetEnemy() ); //qck: Get distance from current enemy

			CBaseEntity* pEnemy = GetEnemy();
			float dist = ( m_vecLastGoalPosition - pEnemy->GetLocalOrigin() ).Length(); //qck: Get enemy distance from defense point. 
			
			if( dist >= 333 ) //TGB: was: 666, this is too large an area
			{
				SetCondition( COND_ZOMBIE_RETURN_TO_POSITION );
				ClearCondition( COND_ENEMY_IN_PERSONAL_SPACE );
				
				Vector forward;
				Vector position = m_vecLastGoalPosition;

				//qck: If this is not the first time you've seen the enemy, run the function.
				if(!(HasCondition( COND_NEW_ENEMY )))
					ReturnToDefending(position, forward);

				SetEnemy(NULL);

				//SetCondition( COND_ZOMBIE_DEFENDING);
			}
			else
			{
				ClearCondition( COND_ZOMBIE_RETURN_TO_POSITION );
				ClearCondition( COND_ZOMBIE_DEFENDING );
				SetCondition( COND_ENEMY_IN_PERSONAL_SPACE );
			}
		}
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
//TGB: force swat schedule
void CNPC_BaseZombie::ZM_ForceSwat ( CBaseEntity *pTarget, bool breakable )
{
	if (!pTarget) return;
	//we're only allowed to do this if we can swat OR we're attacking a breakable
	if (!breakable && !CanSwatPhysicsObjects()) return;

	//TGB: prevent clickspamming ZM from resetting fastzombie's attack process
	if ( IsCurSchedule(GetScheduleID("SCHED_FASTZOMBIE_MELEE_ATTACK1")))
		return;

	m_bSwatBreakable = breakable;
	
	//set the target (phys)obj
...
//-----------------------------------------------------------------------------
// Purpose: Tell NPC to return to defending his position
// Input  :
// Output :
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::ReturnToDefending(const Vector &targetPos, const Vector &traceDir) 
{
	...
	trace_t tr;
	AI_TraceHull( chasePosition, vUpBit, GetHullMins(), 
		GetHullMaxs(), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
	...
	SetSchedule(SCHED_ZOMBIE_RETURN_TO_POSITION);
...
void CNPC_BaseZombie::PrescheduleThink( void )
{
	...
	//qck: The following is a tad hackish
	Vector vecOrigin = GetAbsOrigin();
	Vector vecGoal = m_vecLastGoalPosition;
	
	//qck: 2D comparison
	vecOrigin.z = 0;
	vecGoal.z = 0; 

	//DevMsg("Zombies selected %i\n", gEntList.m_ZombieSelected.Count());
	
	if(IsCurSchedule(SCHED_ZOMBIE_RETURN_TO_POSITION))
	{
		if(vecOrigin == vecGoal) 
		{
			SetCondition(COND_ZOMBIE_DEFENDING);
		}
	...
	//qck: If we have an ambush point, do some ambush stuff. 
	if(m_bIsInAmbush && m_bSwarmAmbushPoint)
	{
		if (!m_pAmbushPoint)
			return; //TGB: whoops, lost our ambush point somewhere along the way

		Vector forward;
		Vector vecGoal = m_pAmbushPoint->GetAbsOrigin();
		...
		m_bIsInAmbush = false;
		m_pAmbushPoint = NULL;
		SetZombieMode(ZOMBIE_MODE_OFFENSE);
	}
}


	//DevMsg("Last Goal Pos: %f, %f, %f\n", vecGoal.x, vecGoal.y, vecGoal.z);
	//DevMsg("Current Origin: %f, %f, %f\n", vecOrigin.x, vecOrigin.y, vecOrigin.z );
#if 0
	...
	/*if ( ( m_flBurnDamageResetTime ) && ( gpGlobals->curtime >= m_flBurnDamageResetTime ) )
	{
		...
		//LAWYER:  Heat field effect
/*
	CBaseEntity *pObject = NULL;
	if (this->IsOnFire())
	{
		if ( m_flNextBurnFrame <= gpGlobals->curtime)
		{	
			while ( ( pObject = gEntList.FindEntityInSphere( pObject, this->GetAbsOrigin(), 64 ) ) != NULL )
			{
				if ((pObject->IsPlayer() || pObject->IsNPC()) && pObject!=this)
				{
					CBaseCombatCharacter *pOther = dynamic_cast<CBaseCombatCharacter *>(pObject);

					//LAWYER:  It's alive.  Give it a minor burnination!
					if (!pOther->IsOnFire()) //LAWYER:  Don't burn something that's already burning itself.
					{
						CTakeDamageInfo info( this, this, 5, DMG_BURN );
	
						if (pObject->GetHealth() <= 20)
						{
			...
			m_flNextBurnFrame = gpGlobals->curtime + 5.0f;
		}
	}
*/ //LAWYER:  This has been cut due to it being an immense strain on resources
//}
...
void CNPC_BaseZombie::StartTask( const Task_t *pTask )
{
	...
	case TASK_FACE_ENEMY:
		{
		//if we're attacking a physobj or breakable we should face it instead of enemy
		if ( m_hPhysicsEnt && 
				(GetEnemy() == NULL ||										//no enemy
				EnemyDistance(GetEnemy()) > ZOMBIE_PLAYER_MAX_SWAT_DIST ||	//enemy far away
				IsCurSchedule(SCHED_ZOMBIE_SWATITEM) ))						//we were standing next to the obj when we started this (most likely ordered to do so by ZM)
				
		{
			DevMsg("Custom facing\n");
			//GetMotor()->SetIdealYawToTarget( m_hPhysicsEnt->GetAbsOrigin() );
			//SetTurnActivity(); 

			Vector vecObjectLoc = m_hPhysicsEnt->GetAbsOrigin();
			if (!FInAimCone( vecObjectLoc ))
			{
				/*
				Vector diff = vecObjectLoc - GetAbsOrigin();
				float yaw = atan2(diff.y, diff.x);
				yaw = (RAD2DEG(yaw) - 90) * -1;

				if (yaw < 0)
					yaw += 360;

				DevMsg("Swat: diff.y = %f | diff.x = %f | yaw = %f | orig = %f\n", diff.y, diff.x, yaw, GetLocalAngles().y);

				GetMotor()->SetIdealYaw(yaw);
*/
				GetMotor()->SetIdealYawToTarget( vecObjectLoc );
				GetMotor()->SetIdealYaw( CalcReasonableFacing( true ) ); // CalcReasonableFacing() is based on previously set ideal yaw

				//TGB: we have to SnapYaw here and immediately complete, or BaseClass will override our stuff in RunTasj
				GetMotor()->SnapYaw();

				TaskComplete();

				//GetMotor()->SetIdealYaw(vecObjectLoc - GetAbsOrigin());
				//GetMotor()->SetIdealYawToTarget( vecObjectLoc );
				//GetMotor()->SetIdealYaw( CalcReasonableFacing( true ) ); // CalcReasonableFacing() is based on previously set ideal yaw
				//SetTurnActivity(); 
			}
...
bool CNPC_BaseZombie::ShouldGib( const CTakeDamageInfo &info )
{
	if ( info.GetDamageType() & (DMG_NEVERGIB|DMG_DISSOLVE) )
		return false;

	//if ( info.GetDamageType() & (DMG_ALWAYSGIB|DMG_BLAST) )
	//TGB: blast damage shouldn't gib, because we have a crazy expire effect triggered by gibbing
	if ( info.GetDamageType() & DMG_ALWAYSGIB )
		return true;
	else if ( info.GetDamageType() & DMG_BLAST )
	{
		return (random->RandomInt(1, 100) == 1); //shh
	}
...
AI_BEGIN_CUSTOM_NPC( base_zombie, CNPC_BaseZombie )
	...
	DECLARE_TASK( TASK_ZOMBIE_DEFEND )
...
//	DECLARE_ACTIVITY( ACT_ZOM_RELEASECRAB )
	...
	DECLARE_CONDITION( COND_ZOMBIE_DEFENDING ) //qck edit
//	DECLARE_CONDITION( COND_ZOMBIE_RELEASECRAB )
	DECLARE_CONDITION( COND_ZOMBIE_LOCAL_MELEE_OBSTRUCTION )
	DECLARE_CONDITION( COND_ZOMBIE_RETURN_TO_POSITION )
	DECLARE_CONDITION( COND_ENEMY_IN_PERSONAL_SPACE )
	DECLARE_CONDITION( COND_ZOMBIE_RETURNING )
	DECLARE_CONDITION( COND_ZOMBIE_DEFENSE_MODE )
	DECLARE_CONDITION( COND_ZOMBIE_IN_AMBUSH )
...
//		"		COND_ZOMBIE_RELEASECRAB"
		"		COND_ENEMY_DEAD"
		"		COND_NEW_ENEMY"
		"		COND_ZOMBIE_RETURN_TO_POSITION" //qck: Just go back to defending if the guy is out of your zone
		"		COND_RECEIVED_ORDERS" //TGB: addition to let this be interrupted when ZM orders to go elsewhere
	)
	...
	(
		...
		"	Interrupts"
//		"		COND_ZOMBIE_RELEASECRAB"
        "       COND_ZOMBIE_RETURN_TO_POSITION" //qck: Don't throw stuff at players who are outside your circle.
		"		COND_ENEMY_DEAD"
		"		COND_NEW_ENEMY"
	)
...
//		"		COND_ZOMBIE_RELEASECRAB"
	...
	(
		...
		"		COND_ZOMBIE_RETURN_TO_POSITION" //qck: STOP CHASING PLEASE
		"		COND_RECEIVED_ORDERS" //qck: Stop chasing if we have new orders.
//		"		COND_ZOMBIE_RELEASECRAB"
	)
...
*/
	...
	//qck: Wait to attack until a player has come into your area
	DEFINE_SCHEDULE
	(
		SCHED_ZOMBIE_DEFEND_POSITION,
		...
		"	"
		"	Interrupts"
		"		COND_ENEMY_IN_PERSONAL_SPACE"
		"		COND_LIGHT_DAMAGE"
		"		COND_RECEIVED_ORDERS" 
	)

	//qck: While in Ambush mode, wait patiently until you are told to move.
	DEFINE_SCHEDULE
	(
	...
	//qck: Special version of SCHED_FORCED_GO used only for telling zombies to return to their defined defense area.
	// Weird animations quirks popped up when using ConqCommand so I made this and ReturnToPosition. 
	DEFINE_SCHEDULE
	(
		SCHED_ZOMBIE_RETURN_TO_POSITION,
		...
		"		TASK_RUN_PATH					0"
		"		TASK_WAIT_FOR_MOVEMENT			0" 
		//"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_ZOMBIE_DEFEND_POSITION"
		"	Interrupts"
		"		COND_ZOMBIE_DEFENDING"
		"		COND_LIGHT_DAMAGE" //qck: Sometimes, depending on where the zombie is in relation to the point they're defending, getting back can be a bit confusing. If they start searching for a way back to no avail at least they'll be able to break out if they're shot. 
		"		COND_RECEIVED_ORDERS" 
	)
	...
	//Very short range wander, used for ZM to avoid unwanted frollicking around
	DEFINE_SCHEDULE
	(
		SCHED_ZOMBIE_WANDER_SHORT,

		"	Tasks"
		"		TASK_STOP_MOVING				0"
		"		TASK_WANDER						10024" // <1 feet to 2 feet
		"		TASK_WALK_PATH					0"
		"		TASK_WAIT_FOR_MOVEMENT			0"
		"		TASK_STOP_MOVING				0"
		"		TASK_WAIT_PVS					0" // if the player left my PVS, just wait.
		"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_ZOMBIE_WANDER_SHORT" // keep doing it
		"	"
		"	Interrupts"
		"		COND_NEW_ENEMY"
		"		COND_SEE_ENEMY"
...
//		"		COND_ZOMBIE_RELEASECRAB"
	...
	(
		...
		""
		"	Interrupts"
		"		COND_NEW_ENEMY"//LAWYER:  Added, so seeing a player will stop the move
		"		COND_ZOMBIE_CAN_SWAT_ATTACK" //TGB: need this for zombie swattage
	)
