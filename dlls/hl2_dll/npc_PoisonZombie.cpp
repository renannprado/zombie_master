...
// The hulk, based on poison zombie
...
// Skill settings.
//-----------------------------------------------------------------------------
ConVar sk_zombie_poison_health( "sk_zombie_poison_health", "150");
ConVar sk_zombie_poison_dmg_spit( "sk_zombie_poison_dmg_spit","20");

//static ConVar zm_popcost_hulk("zm_popcost_hulk", "4", FCVAR_NOTIFY | FCVAR_REPLICATED, "Population points taken up by hulks");
...
//	virtual Vector HeadTarget( const Vector &posSrc );
	...
	bool CanSwatPhysicsObjects( void ) { return false; } //TGB: for now
	//
...
//	virtual int RangeAttack2Conditions( float flDot, float flDist );
	...
	virtual float GetClawAttackRange() const { return 80; } //70 //TGB: increase for largeyhulkie
	...
	int GetPopCost() { return zm_popcost_hulk.GetInt(); }
	int	GetZombieType() { return TYPE_HULK; }
protected:
...
//if we ever change entity names, remember to mirror it to all the places GetClassname is used to check for this Z
...
void CNPC_PoisonZombie::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_PoisonZombie::DeathSound(const CTakeDamageInfo &info )
{
	if ( !( info.GetDamageType() & ( DMG_BLAST | DMG_ALWAYSGIB) ) ) 
	{
		EmitSound( "NPC_PoisonZombie.Die" );
