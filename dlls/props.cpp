...
void CBreakableProp::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
	...
	//TGB: objective items can catch on fire (0000390)
	//real problem: they never stop burning because they're invincible (unless the mapper is stupid)
	//eternally burning items are basically always bad, so just turn off igniting for them
...
int CBreakableProp::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	//TGB: if we are explodable do special things
	if (m_explodeDamage > 0 || m_explodeRadius > 0)
	{
		//explodable AND being carried AND being attacked by a player, don't take damage (0000380)
		IPhysicsObject *pPhysics = VPhysicsGetObject();
		CBaseEntity *attacker = info.GetAttacker();

		if (pPhysics && attacker &&	
			pPhysics->GetGameFlags() & FVPHYSICS_PLAYER_HELD &&		//being carried?
			attacker->IsPlayer() && attacker->GetTeamNumber() == 2)	//attacker is a survivor?
		{
			return 0; //attacker is a player on the survivor team
		}
		...
		if (IsOnFire())
		{
			//DevMsg("Prop scaled firedmg up from %f to %f\n", info.GetDamage(), info.GetDamage() * 2.0f );
...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if ( m_explodeDamage > 0 || m_explodeRadius > 0 )
	{
		...
		bExploded = true;
		//LAWYER:  Dump player details here
		CBasePlayer *pPlayer = dynamic_cast<CBasePlayer*>(pAttacker);
		if (pPlayer)
		{
			UTIL_ClientPrintAll( HUD_PRINTCONSOLE, "SteamID %s1 (%s2) popped a barrel.\n", pPlayer->GetNetworkIDString(), pPlayer->GetPlayerName());
			/*if (GetOwnerEntity())
			{
				DevMsg("Barrel owner was %s\n", GetOwnerEntity()->GetClassname());
			}
			else
				DevMsg("Barrel not owned\n");
			*/
		}
	}
