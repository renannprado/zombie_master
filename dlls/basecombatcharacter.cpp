...
#ifdef HL2_DLL
//extern int	g_interactionBarnacleVictimReleased;  //LAWYER:  No barnacles
#endif //HL2_DLL
...
ConVar zm_ragdoll_force( "zm_ragdoll_force", "2", FCVAR_ARCHIVE, "Force multiplier for ragdolling" );
#ifndef _RETAIL
...
IMPLEMENT_SERVERCLASS_ST(CBaseCombatCharacter, DT_BaseCombatCharacter)
	...
	SendPropBool( SENDINFO(m_bConqSelected)), //LAWYER:  Networked class
	SendPropFloat( SENDINFO(m_flHealthRatio), 6, SPROP_ROUNDUP, 0.0f, 1.0f), //TGB: networking this for clientside healthcircles
END_SEND_TABLE()
...
#endif // HL2_DLL*/ //LAWYER:  No barnacles
	return false;
}
...
CBaseCombatCharacter::CBaseCombatCharacter( void )
{
	...
	//LAWYER:  Initialise the selected bits
...
void CBaseCombatCharacter::Precache()
{
	...
	PrecacheScriptSound( "NPC_Antlion.RunOverByVehicle" );//tgb
...
bool CBaseCombatCharacter::CorpseGib( const CTakeDamageInfo &info )
{
	...
	if ( HasHumanGibs() )
	{
		...
		//TGB: in practice these are more skulls
		//CGib::SpawnRandomGibs( this, 4, GIB_HUMAN );	// throw some human gibs.
		gibbed = true;
		//TGB:
		Vector vecTraceDir = GetAbsOrigin() + Vector ( 0 , 0 , 20 );//move up a bit, and trace down.
		const float flNoise = 5.0;
		for (int i = 0 ; i < 4 ; i++ )
		{
			...
			UTIL_TraceLine ( vecTraceDir, vecTraceDir + Vector ( 0, 0, -48 ),  MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
...
Vector CBaseCombatCharacter::CalcDamageForceVector( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageForce() != vec3_origin || (info.GetDamageType() & DMG_NO_PHYSICS_FORCE) )
	{
		...
		{
			//TGB: let's try a bit more force here for MAXIMUM DAMAGE
			Vector forceScaled = info.GetDamageForce() * zm_ragdoll_force.GetFloat();
...
void CBaseCombatCharacter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( VPhysicsGetObject() )
	{
		...
		//TGB: let's try just dropping all weapons here
...
/*#ifdef HL2_DLL
...
#endif*/ //LAWYER:  I commented this out for a reason, but I'll be damned if I can remember why
	...
	//TGB: reset health ratio, or players will have a red circle on respawn
	m_flHealthRatio = 1.0f;
}
	...
	const Vector &vecForward, const QAngle &vecAngles, float flDiameter )
{
	...
	//TGB: transfer our ammo into a hidden compartment inside the gun
	//	copied over from Weapon_Drop as this wasn't happening for guns dropped on death
	const int type = pWeapon->GetPrimaryAmmoType();
	pWeapon->m_iStoredPrimaryAmmo = GetAmmoCount( type );
	SetAmmoCount( 0, type ); //zero our reserves


	//vecThrow *= random->RandomFloat( 400.0f, 600.0f );
	//TGB: that was overly fast for our purposes
	vecThrow *= random->RandomFloat( 30.0f, 60.0f );
...
void CBaseCombatCharacter::Weapon_DropAll( bool bDisallowWeaponPickup )
{
	if ( GetFlags() & FL_NPC )
	{
		for (int i=0; i<MAX_WEAPONS; ++i) 
		{
			...
			/*
			//make sure we don't drop carrywep
			if (strcmp(pWeapon->GetClassname(), "weapon_zm_carry") == 0)
				continue;
			//LAWYER:  Or the fists
			if (strcmp(pWeapon->GetClassname(), "weapon_zm_fists") == 0)
				continue;
			*/
	...
	if ( pActiveWeapon )
	{
		/*const char *name = pActiveWeapon->GetClassname();
		if (Q_stricmp(name, "weapon_zm_carry") == 0 ||
			Q_stricmp(name, "weapon_zm_fists") == 0)
			return;*/
		if (pActiveWeapon->CanBeDropped() == false)
			return;
		...
		vecThrow *= random->RandomFloat( 40.0f, 70.0f );
...
void CBaseCombatCharacter::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	//TGB: we have our own ways for giving ammo
/*
...
*/
	if (pWeapon->m_iStoredPrimaryAmmo > 0)
	{
		GiveAmmo( pWeapon->m_iStoredPrimaryAmmo, pWeapon->m_iPrimaryAmmoType );
		pWeapon->m_iStoredPrimaryAmmo = -1;
	}
...
int CBaseCombatCharacter::TakeHealth (float flHealth, int bitsDamageType)
{
	...
	const int result = BaseClass::TakeHealth(flHealth, bitsDamageType);

	//TGB: update the health ratio
	// clamping to avoid networking-related rounding warnings
	m_flHealthRatio = min(1.0f, (float)GetHealth() / (float)GetMaxHealth());
...
//		return FindEntityRelationship( pTarget )->disposition;
		if (pTarget->IsPlayer()) //LAWYER:  Crazy code to make NPCs hate the players!  This needs expanding, to check if the player is actually a Zombie Master
		{
			CHL2MP_Player *pPlayer = NULL;
			pPlayer = static_cast< CHL2MP_Player * >( pTarget );
			if (pPlayer)
			{
				if (pPlayer->GetTeamNumber() == 2)
				{
					return D_HT; //LAWYER:  Only hate team 2! (fix me!)
				}
			}
		}
	}
...
int CBaseCombatCharacter::GiveAmmo( int iCount, int iAmmoIndex, bool bSuppressSound)
{
	...
	//LAWYER:  Check for the ammo requirements thing
	const int iSpace = iMax - GetAmmoCount( iAmmoIndex );
	//TGB: only do this if 25% actually means something, else weapons with 1 max ammo will always be denied ammo
	if (iMax > 3 && iSpace <= ceil(iCount / 4.0)) //less than 25% is a bad thing. TGB: ceil'd to err on the side of "first shoot some more bullets, son"
	{
		return 0; //will show an indicator that it can't be nabbed
	}
