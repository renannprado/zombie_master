...
//
// $Workfile:     $
// $Date: 2006-11-23 23:26:44 $
//
//-----------------------------------------------------------------------------
// $Log: sdk_bot_temp.cpp,v $
// Revision 1.4  2006-11-23 23:26:44  Angry_Lawyer
// *** empty log message ***
//
// Revision 1.3  2006-10-26 18:02:49  Angry_Lawyer
// *** empty log message ***
//
// Revision 1.2  2006-06-30 17:11:42  qckbeam
// Ambush mode
//
// Revision 1.1  2006-06-01 16:01:21  tgb
// Initial full source add, take 2
//
// Revision 1.2  2006-05-31 14:40:20  tgb
...
//
// Revision 1.5  2006/02/11 16:39:42  qckbeam
// no message
//
// Revision 1.4  2006/02/11 13:50:31  qckbeam
// no message
//
// Revision 1.3  2006/02/10 16:23:09  qckbeam
// no message
//
// Revision 1.2  2006/02/10 12:58:59  qckbeam
// Added testing bots
//
// Revision 1.1.1.1  2005/10/11 14:57:58  theGreenBunny
...
{
	char botname[ 64 ];
	Q_snprintf( botname, sizeof( botname ), "L_Ron_Hubbard%02i", g_CurBotNumber );
	...
	//TGB: bots shouldn't become ZM
	pPlayer->m_bDontWannaZM = true;

	pPlayer->SetModel("models/male_pi.mdl");

	//pPlayer->ChangeTeam( TEAM_UNASSIGNED ); 
	pPlayer->RemoveAllItems( true );
	pPlayer->Spawn();
	pPlayer->GiveNamedItem("weapon_zm_fists");
	pPlayer->GiveNamedItem("weapon_zm_carry");
	pPlayer->Weapon_Switch( pPlayer->Weapon_OwnsThisType("weapon_zm_fists") );
...
//------------------------------------------------------
//qck: Give our bot a weapon, and force him to equip it. 
//------------------------------------------------------
void CC_Bot_SwitchWeapon()
{
	CBaseEntity *pEntity = NULL;
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
	CBaseCombatWeapon *pWeapon = pPlayer->GetActiveWeapon();

	while ( (pEntity = GetNextCommandEntity( pPlayer, engine->Cmd_Argv(2), pEntity )) != NULL )
	{
		if( CSDKBot *pBot = (CSDKBot*)pEntity)
		{
			//qck: weird things happen when they don't do this, for some strange reason
			pBot->RemoveAllWeapons(); 

			if(pBot->Weapon_CanSwitchTo( pWeapon ))
			{
				pBot->Weapon_Equip( pWeapon );
			}
			else
				Msg("%s can not equip that weapon!\n", pBot->GetPlayerName());
		}
	}
}

ConCommand bot_switchweapon("bot_switchweapon", CC_Bot_SwitchWeapon, "Force the bot to switch to the weapon you are holding");
