...
ConVar	teamplay( "mp_teamplay","1", FCVAR_NOTIFY ); //ZM is teamplay
...
//LAWYER:  ZM Convars
ConVar	roundlimit( "zm_roundlimit","0", FCVAR_NOTIFY, "Sets the number of rounds before the server changes map\n" );
//TGB: zombie limit
ConVar zm_zombiemax("zm_zombiemax", "50", FCVAR_ARCHIVE|FCVAR_NOTIFY|FCVAR_REPLICATED, "Sets maximum population count the ZM is allowed to have active at once. Certain zombies count for more pop than others, see zm_popcost_shambler etc.");
//LAWYER:  Banshee limit
//ConVar zm_bansheemax("zm_bansheemax", "0.0", FCVAR_ARCHIVE|FCVAR_NOTIFY, "Sets maximum number of banshees per player the ZM is allowed to have active at once. Set to 0 to remove the cap. Disabled by default since new population system was introduced that in practice includes a banshee limit.");
ConVar zm_banshee_limit("zm_banshee_limit", "-1", FCVAR_ARCHIVE|FCVAR_NOTIFY, "Sets maximum number of banshees per survivor that the ZM is allowed to have active at once. Set to 0 or lower to remove the cap. Disabled by default since new population system was introduced that in practice includes a banshee limit.");
