...
//=============================================================================
//LAWYERESQUE MOVEMENTS!
//-----------------------------------------------------------------------------
// Purpose: Force-march NPCs to a location
// Input  :
// Output :
//-----------------------------------------------------------------------------
void CAI_BaseNPC::ConqCommanded(const Vector &targetPos, const Vector &traceDir, bool bRun, CAI_BaseNPC *npc) 
{
	...
	npc->m_vecLastPosition = chasePosition;
	npc->m_vecLastGoalPosition = chasePosition; //qck: Would have used above if I was sure vecLastPosition was not changed anywhere else.

	if (npc->m_hCine != NULL)
	{
		npc->ExitScriptedSequence();
	}
/*TGB: unneeded now that we're not watching for physobj while walking
	CNPC_BaseZombie *zombie = dynamic_cast< CNPC_BaseZombie * >(npc);
	if ( zombie && zombie->CanSwatPhysicsObjects() )
	{
		zombie->SetSchedule( SCHED_ZOMBIE_FORCED_SWAT_GO );
		return;
	}
*/
	if ( bRun )
