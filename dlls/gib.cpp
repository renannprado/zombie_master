...
void CGib::SpawnHeadGib( CBaseEntity *pVictim )
{
	...
	if ( pVictim )
	{
		...
		{
			vecNewVelocity = Vector (random->RandomFloat(-50,50), random->RandomFloat(-50,50), random->RandomFloat(50,200)); //tgb: lowered numbers
		}
		...
		pGib->SetNextThink(gpGlobals->curtime + 2.0f);
		pGib->SetThink(&CBaseEntity::SUB_Remove);
		pGib->m_nRenderFX = kRenderFxFadeSlow;

		//UTIL_BloodImpact(pGib->GetAbsOrigin(), RandomVector(0.0f, 1.0f), BLOOD_COLOR_RED, 50);

		UTIL_BloodSpray(pVictim->GetAbsOrigin() + Vector(0, 0, 10), Vector(0, 0, 1), BLOOD_COLOR_RED, 10, FX_BLOODSPRAY_GORE | FX_BLOODSPRAY_CLOUD );
	}
	pGib->LimitVelocity();
}
...
void CFlamingGib::Spawn( const char *szGibModel )
{
	//SetThink( BurnThink );
	BaseClass::Spawn( szGibModel );
}
//qck: Gib of firey death
void CFlamingGib::Think()
{
	//qck: This behaves in a _really_ weird way. It seems to be called once while it's active, and then the rest of the time when it's been killed off. Weird. 
	//Msg("Gibbers!\n"); TGB: this is extremely annoying if you're not interested in this message, like-a me
	SetNextThink( gpGlobals->curtime );
