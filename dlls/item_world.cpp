...
#define ITEM_PICKUP_BOX_BLOAT		16
...
void CItem::FallThink ( void )
{
	...
	//TGB: make sure we ALWAYS have a valid spawn origin, even if it's in mid-air
	if (m_vOriginalSpawnOrigin.IsZero())
		m_vOriginalSpawnOrigin = GetAbsOrigin();
...
{
	if (pOther->GetTeamNumber() == 3)
	{
		return; //LAWYER:  You can't pick stuff up as a Zombiemaster
	}
...
{ //LAWYER:  Add checks for camping players
	SetTouch( NULL );
	...
	//TGB: if we are going to get moved to (0,0,0), we should probably not do so
	Vector newpos = g_pGameRules->VecItemRespawnSpot( this );
	if (newpos.IsZero())
		newpos = GetAbsOrigin();

	UTIL_SetOrigin( this, newpos );// blip to whereever you should respawn.
...
{
	//LAWYER:  Objects don't respawn when being camped.
	CBaseEntity *pIterated = NULL;

	while ( (pIterated = gEntList.FindEntityInSphere( pIterated, this->GetAbsOrigin(), sv_respawnrange.GetFloat() )) != NULL ) //Search for players
	{
		if (pIterated->IsPlayer())
		{
