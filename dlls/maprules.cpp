...
void CGameScore::InputApplyScore( inputdata_t &inputdata )
{
	...
	{
		//TGB: problem: activator may be an object held by a player, in which case we want to award the player

		//a carried object has a physobj
		IPhysicsObject *pObj = pActivator->VPhysicsGetObject();	
		if ( pObj && pObj->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
		{
			//the owner of the baseent is the holder of the object
            CBaseEntity *pHolder = pActivator->GetOwnerEntity();

			if (pHolder != NULL) //pass activator status on to holder
				pActivator = pHolder;
			else
				return;	
			...
			return; //not a held physobj and not a player = unusable
	}

	if ( CanFireForActivator( pActivator ) == false )
		return;

	//always a player if we get here
	if ( AwardToTeam() )
	{
	...
	HL2MPRules()->PlayerGotPoints( pActivator, Points(), this );
}
...
//LAWYER:  Additional scoring logic
...
void CGameScoreTeam::Spawn( void )
{
	int iScore = Points();
	BaseClass::Spawn();
	SetPoints( iScore );
...
bool CGameScoreTeam::KeyValue( const char *szKeyName, const char *szValue )
{
	if (FStrEq(szKeyName, "points"))
	{
		SetPoints( atoi(szValue) );
...
void CGameScoreTeam::ApplyTheScore(bool bZM)
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( plr )
		{
			if (plr->IsZM() == bZM && !plr->IsObserver())
			{
				plr->AddPoints( Points(), AllowNegativeScore() );

				HL2MPRules()->PlayerGotPoints( plr, Points(), this );
			}
		}
	}
}

void CGameScoreTeam::InputApplyScoreSurvivors( inputdata_t &inputdata )
{
	ApplyTheScore(false);
}

void CGameScoreTeam::InputApplyScoreZM( inputdata_t &inputdata)
{
	ApplyTheScore(true);
}
...
private:
	...
	COutputEvent m_OnAllPlayersInZone;
};
...
//	DEFINE_INPUTFUNC(FIELD_VOID, "CountPlayersInZone", InputCountPlayersInZone),
	...
	DEFINE_OUTPUT(m_OnAllPlayersInZone, "OnAllPlayersInZone"), //TGB: Added because pimsy wants it
...
//Spawn function, because we need to initialise the think function
void CGamePlayerZone::Spawn()
{
	SetThink ( &CGamePlayerZone::CountThink );
...
//			Lawyeredit: It now runs this every second
//-----------------------------------------------------------------------------
void CGamePlayerZone::CountThink( void )
{

	int playersInCount = 0;
	int playersOutCount = 0;
	//TGB: we need these for calculating percentage of players
	int playersTotal = 0;
	double playersPercentage = 0.0;
...
//		return;
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer )
		{
			playersTotal++; //TGB: count number of players
			//LAWYER:  Okay, the way we're going to do it is by detecting mins and maxes
			//of the box, and the origin of the player
			//X mins
			Warning ("%i, %i, %i\n", this->WorldAlignMins().x, this->WorldAlignMins().y, this->WorldAlignMins().z);
			Warning ("%i, %i, %i\n", this->WorldAlignMaxs().x, this->WorldAlignMaxs().y, this->WorldAlignMaxs().z);
			Warning ("%i, %i, %i\n", this->WorldAlignMaxs().x, this->WorldAlignMaxs().y, this->WorldAlignMaxs().z);
			
/*
			if ((pPlayer->GetAbsOrigin().x < this->WorldAlignMins().x) ||
				(pPlayer->GetAbsOrigin().y < this->WorldAlignMins().y) ||
				(pPlayer->GetAbsOrigin().z < this->WorldAlignMins().z) ||
				(pPlayer->GetAbsOrigin().x > this->WorldAlignMaxs().x) ||
				(pPlayer->GetAbsOrigin().y > this->WorldAlignMaxs().y) ||
				(pPlayer->GetAbsOrigin().z > this->WorldAlignMaxs().z))
			{
				//Zooomj, can't do it captin!
				playersOutCount++;
			}
			else
			{
...
/*
	...
	//TGB: alone? don't subtract ZM
	if (playersTotal > 1)
		playersTotal--; //total players minus the ZM

	/*TGB: When this is all confirmed as working correctly, we should nest this whole thing
	behind a (playersTotal > 1) check to keep a player who is messing on his own as ZM from triggering this*/
//	Warning("Current number of players in the zone is %i \n", playersInCount);
//	Warning("Current number of players out the zone is %i \n", playersOutCount);
	//TGB: calc percentage and output. I'd prefer the percentage as an input/property but don't know how.
/*	playersPercentage = playersInCount / playersTotal;
	if (playersPercentage >= 0.9)
	{
		m_OnAllPlayersInZone.FireOutput( pPlayer, this);
	}
	...
	*//*
	SetNextThink( gpGlobals->curtime + 1.0f ); //THINK AGAIN, MUNRO!
}
*/
