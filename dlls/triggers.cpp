...
//LAWYER:  The following were added to count players
//-----------------------------------------------------------------------------
// Purpose: Counts all players touching it, and will fire when it reaches
//			a map set limit.
//-----------------------------------------------------------------------------
class CTriggerPlayerCount : public CBaseTrigger
{
public:


	DECLARE_CLASS( CTriggerPlayerCount, CBaseTrigger );
...
BEGIN_DATADESC( CTriggerPlayerCount )
...
LINK_ENTITY_TO_CLASS( trigger_playercount, CTriggerPlayerCount );


//-----------------------------------------------------------------------------
// Purpose: Called when spawning, after keyvalues have been handled.
//-----------------------------------------------------------------------------
void CTriggerPlayerCount::Spawn( void )
{
	BaseClass::Spawn();

	InitTrigger();

	SetThink( &CTriggerPlayerCount::CountThink );
	SetNextThink( gpGlobals->curtime );
}
...
//-----------------------------------------------------------------------------
void CTriggerPlayerCount::CountThink( void )
{
	if (m_bActive == false)
			return;
	...
	if ( root )
	{
		for ( touchlink_t *link = root->nextLink; link != root; link = link->nextLink )
		{
			...
			if ( pTouch )
			{
				CBasePlayer *pPlayer = dynamic_cast<CBasePlayer *>(pTouch);
	...
	int numplayers = GetGlobalTeam(2)->GetNumPlayers();
	
	if (numplayers > 0) //LAWYER:  Fixed the integer divide by zero.
	{
		float fPercentage = ((float)count/(float)numplayers)*100;
//		Warning("%f versus %i\n", fPercentage, m_iPercentageToFire);

		if (fPercentage >= m_iPercentageToFire)
		{
//			Warning("FIRE!");
			m_OnCount.FireOutput(NULL, this);  //Fire outputs when triggered.
		}
	}
...
void CTriggerPlayerCount::InputToggle( inputdata_t &inputData )
{
	...
	else
	{
		m_bActive = true;
		SetThink( &CTriggerPlayerCount::CountThink );
		SetNextThink( gpGlobals->curtime + 1.0f ); //LAWYER:  Set a think time!
	}

}

void CTriggerPlayerCount::InputDisable( inputdata_t &inputData )
{
		m_bActive = false;
}

void CTriggerPlayerCount::InputEnable( inputdata_t &inputData )
{
		m_bActive = true;
		SetThink( &CTriggerPlayerCount::CountThink ); //LAWYER:  Remember to think!
		SetNextThink( gpGlobals->curtime + 1.0f );
}
//-----------------------------------------------------------------------------
// Purpose: Counts all ents touching it, and will fire when it reaches
//			a map set limit.  Uses solid numbers, as opposed to percentages
//-----------------------------------------------------------------------------
class CTriggerEntityCount : public CBaseTrigger
{
public:


	DECLARE_CLASS( CTriggerEntityCount, CBaseTrigger );
	...
	int		m_iCountToFire; //LAWYER:  The percentage to fire this beyotch
	bool	m_bActive;
	int		m_iTriggerFlags; //LAWYER:  What kinds of entity trigger this

	COutputEvent m_OnCount;
	COutputEvent m_OnNotCount;
};

BEGIN_DATADESC( CTriggerEntityCount )

	// Function Pointers
	//DEFINE_FUNCTION( CountThink ),

	// Fields
	DEFINE_KEYFIELD( m_iCountToFire, FIELD_INTEGER, "counttofire"),
	DEFINE_KEYFIELD(m_bActive, FIELD_BOOLEAN, "Active"), 
...
LINK_ENTITY_TO_CLASS( trigger_entitycount, CTriggerEntityCount );


//-----------------------------------------------------------------------------
// Purpose: Called when spawning, after keyvalues have been handled.
//-----------------------------------------------------------------------------
void CTriggerEntityCount::Spawn( void )
{
	BaseClass::Spawn();

	InitTrigger();

//	SetThink( &CTriggerEntityCount::CountThink );
//	SetNextThink( gpGlobals->curtime );
}

//-----------------------------------------------------------------------------
// Purpose: Counts the number of players
//-----------------------------------------------------------------------------
void CTriggerEntityCount::InputCount( inputdata_t &inputData )
{
	if (m_bActive == false)
			return;
	...
	if ( root )
	{
		for ( touchlink_t *link = root->nextLink; link != root; link = link->nextLink )
		{
			...
			if ( pTouch )
			{
				//LAWYER:  Switch between different trigger flavours!
				if (m_iTriggerFlags == 1) //Players...
				{
					CBasePlayer *pPlayer = dynamic_cast<CBasePlayer *>(pTouch);
				...
				else if (m_iTriggerFlags == 2) //NPCs..
				{
					CAI_BaseNPC *pNPC = dynamic_cast<CAI_BaseNPC *>(pTouch);
					if (pNPC)
					{
						count++; //LAWYER:  Any NPCs can trigger this
					}
				}
				else if (m_iTriggerFlags == 3) //Prop_physics, func_physbox, prop_physics_override
				{
					CPhysicsProp *pProp = dynamic_cast<CPhysicsProp *>(pTouch);
					if (pProp)
					{
						count++; //LAWYER:  point-based props
					}
					CPhysBox *pBox = dynamic_cast<CPhysBox *>(pTouch);
					if (pBox)
					{
						count++; //LAWYER:  brush-based props
					}
/*					CPhysBoxMultiplayer *pBoxMult = dynamic_cast<CPhysBoxMultiplayer *>(pBoxMult);
					if (pBoxMult)
					{
						count++; //LAWYER:  subtype of brush props
					}
					CPhysicsPropMultiplayer *pPropMult = dynamic_cast<CPhysicsPropMultiplayer *>(pPropMult);
					if (pPropMult)
					{
						count++; //LAWYER:  subtype of point props
					}*/

				}
				else
				{
					Warning("Invalid flag for Trigger_Entitycount!\n");
				}
			}
		}
		...
		{
//			Warning("Zooooooooooooooomj! Firing!\n");
			m_OnCount.FireOutput(NULL, this);  //Fire outputs when triggered.
		}
		else
		{
//			Warning("Zooooooooooooooomj! Not Firing!\n");
			m_OnNotCount.FireOutput(NULL, this);  //Do the opposite!
		}
...
void CTriggerEntityCount::InputToggle( inputdata_t &inputData )
{
	// Toggle our active state
	if ( m_bActive )
	{
...
void CTriggerEntityCount::InputDisable( inputdata_t &inputData )
{
		m_bActive = false;
}

void CTriggerEntityCount::InputEnable( inputdata_t &inputData )
{
		m_bActive = true;
}

//-----------------------------------------------------------------------------
// Purpose: Gives points to all (human) players inside it when triggered
//-----------------------------------------------------------------------------
class CTriggerGivePoints : public CBaseTrigger
{
public:


	DECLARE_CLASS( CTriggerGivePoints, CBaseTrigger );
	...
	int		m_iPoints; //LAWYER:  The number of points to dish out

};

BEGIN_DATADESC( CTriggerGivePoints )
...
LINK_ENTITY_TO_CLASS( trigger_givepoints, CTriggerGivePoints );


//-----------------------------------------------------------------------------
// Purpose: Called when spawning, after keyvalues have been handled.
//-----------------------------------------------------------------------------
void CTriggerGivePoints::Spawn( void )
{
	BaseClass::Spawn();
...
//-----------------------------------------------------------------------------
// Purpose: Awards points to survivors in the area
//-----------------------------------------------------------------------------
void CTriggerGivePoints::InputAward( inputdata_t &inputdata )
{
		...
		for ( touchlink_t *link = root->nextLink; link != root; link = link->nextLink )
		{
			...
			if ( pTouch )
			{
				...
				if ( pPlayer )
				{
					...
					if (pPlayer->IsSurvivor())
					{
						//LAWYER:  Award some points!
						pPlayer->IncrementFragCount(inputdata.value.Int());
					}
				}
			}
		}
