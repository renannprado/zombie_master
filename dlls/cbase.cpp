...
CBaseEntityOutput::~CBaseEntityOutput()
{
	...
	//if (m_ActionList != NULL) DevMsg("m_ActionList==%p\n", m_ActionList);

	/*
	TGB: attempt to organize comments into list

	- The zombie vs. barrel crash's direct cause is m_ActionList being 00000001, which is a garbage address but not NULL
	- Trying to use this pointer will crash it, because it's an invalid addy
	- m_ActionList gets this garbage value by magic, none of the actionlist assignments set it as such (ie. there is no m_ActionList = banana where it's set to 00000001, checked with devmsg at every assignment)
	- m_ActionList does not get this value from some kind of leftover after deletion here, as it comes into this function as 00000001 despite being set to NULL in the last run through the function

	So why does it crash here? I have no idea. I can only speculate that the bug is somewhere else and this is a side effect. A very weird side effect.
    */

	/*
	TGB: As it turns out the bug was related to the rally point code, specifically the declaration of a variable there. Weird stuff.
	*/
