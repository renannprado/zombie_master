...
//#define INITIAL_POOL		100 //LAWYER:  Initial pool value
ConVar	zm_initial_resources( "zm_initial_resources","100" ); //TGB: initial zm sauce cvar
//----------------------------------------------------
...
CBasePlayer::CBasePlayer( )
{
	...
	m_bHoldingObject = false; //qck: Holding flag

	//### SPK ### Added fix
	// this is/was causing random crashs in DestroyPlayerController
	// because m_pPhysicsController was not initialized
	//m_pPhysicsController = NULL; //TGBMERGENOTE: copied back in together with holdingobject line but commented out for now
	//### END SPK ###
	//LAWYER:  I'm losing track of who I owe beers to.
...
//qck: added for some checks in weapon code
bool CBasePlayer::IsCarryingSomething()
{
	return m_bHoldingObject;
}
...
ConVar sv_unlag_banshee_only( "sv_unlag_banshee_only", "0", 0, "Enables banshee-only mode for lagcompensation. Other zombies need it less, and this might prevent bugs." );
...
bool CBasePlayer::WantsLagCompensationOnEntity( const CBaseEntity *pEntity, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
	...
	//TGB: for if we only care about banshees
	if (sv_unlag_banshee_only.GetBool())
	{
		CNPC_BaseZombie *zombie = dynamic_cast<CNPC_BaseZombie *>((CBaseEntity *)pEntity);
		if (zombie && zombie->GetZombieType() != CNPC_BaseZombie::TYPE_BANSHEE)
			return false;
	}
...
{
	pl.v_angle.GetForModify() = viewAngles;	//TGB: 0000069 spectator first person fix
	pl.fixangle = FIXANGLE_ABSOLUTE;
}
...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( event )
	{
		...
		//TGB: extended to include information on zombie attacker, so server plugins can track zombie's damage output/effectiveness/etc
		if ( attacker->IsPlayer() )
		{
			CBasePlayer *player = ToBasePlayer( attacker );
			event->SetInt("attacker", player->GetUserID() ); // hurt by other player
			event->SetBool("zombie", false);
		}
		else if ( attacker->IsNPC() )
		{
			event->SetInt("attacker", attacker->entindex());
			event->SetBool("zombie", true);
		}
		else
		{
...
void CBasePlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	//LAWYER:  Do a check for the Human team being dead.
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if (m_lifeState == LIFE_DEAD)
	{
		...
		//if (fAnyButtonDown) //LAWYER:  This bit broke respawn code
		//	return; //As I can't see any real reason to stop transition to spectators from holding down the scoresheet
...
// choose to respawn.
	...
	{
		// go to dead camera. 
		ChangeTeam(TEAM_SPECTATOR); //LAWYER:  Switch over to the Observer team.
...
// wait for any button down,  or mp_forcerespawn is set and the respawn time is up
	...
	//Msg( "Respawn\n");
	//LAWYER:  No respawning, unless I say so!
...
//	pl.deadflag = true; //LAWYER:  Removed, for some reason
...
void CBasePlayer::CheckObserverSettings()
{
	...
	if (  m_iObserverMode == OBS_MODE_IN_EYE || m_iObserverMode == OBS_MODE_CHASE )
	{
		...
		{
			//TGB: it's annoying to spectate a zombie, and get switched away to a random dude when
			//	it dies. Seeing as we don't know if our old target was a zombie unless we add a
			//	hack for that, we can't have a special case. That's not too bad, because players
			//	can just click to switch to the next player if that's what they want.
			...
			//{
				...
				if ( mp_forcecamera.GetInt() == OBS_ALLOW_ALL )
				{
					...
					//do find a next target so the jump key does something useful
					m_hObserverTarget.Set(FindNextObserverTarget(false));
...
*/
			...
			//TGB: for peace of mind, make sure target is player or npc
			if ((target->IsPlayer() == false && target->IsNPC() == false)
...
bool CBasePlayer::IsValidObserverTarget(CBaseEntity * target)
{
	...
	if ( target->IsPlayer() == false && target->IsNPC() == false )	// only track players, TGB: or zombies
		return false;

	//TGB: player specific checks:
	...
	if ( target->IsEffectActive( EF_NODRAW ) ) // don't watch invisible players
		return false;

	if ( target->m_lifeState == LIFE_RESPAWNABLE ) // target is dead, waiting for respawn
		return false;

	if ( target->m_lifeState == LIFE_DEAD || target->m_lifeState == LIFE_DYING )
	{
		// in case it's a respawning player (though we don't have any (or do we? upgraaaades :fist:))
		if (player && ((player->m_flDeathTime + DEATH_ANIMATION_TIME ) < gpGlobals->curtime ))
		{
			return false;	// allow watching until 3 seconds after death to see death animation
		}
		else if (target->IsAlive() == false && target->IsNPC())
		{
			return false;
		}
...
bool CBasePlayer::CanPickupObject( CBaseEntity *pObject, float massLimit, float sizeLimit )
{
	...
	if ( checkEnable )
	{
		//TGB: zm has no bouncey bombey types
...
{
	if (GetTeamNumber() == TEAM_SPECTATOR || GetTeamNumber() == 3)
		ucmd->buttons &= ~IN_USE; //LAWYER:  Zombiemasters and Spectators can't use things

	m_touchedPhysObject = false;

	if ( pl.fixangle == FIXANGLE_NONE)
	{
		VectorCopy ( ucmd->viewangles, (QAngle)pl.v_angle.Get() );	//TGB: 0000069 spectator first person fix
	}
		...
		(developer.GetInt() == 0 && gpGlobals->eLoadType == MapLoad_NewGame && gpGlobals->curtime < 3.0 ) )
	{
		...
		ucmd->buttons = 0;
		ucmd->impulse = 0;
		VectorCopy ( pl.v_angle.GetForModify(), ucmd->viewangles );	//TGB: 0000069 spectator first person fix
	}
#ifdef _XBOX
	else
	{
...
{
	//TGB: NOT USED! Only HL2MP_Player::PreThink and HL2_Player::PreThink are called
	...
	//LAWYER:  Add things here for Zombie Master resource pool refilling.
...
void CBasePlayer::CheckTimeBasedDamage() 
{
	...
	//LAWYER:  We can snead something in here
//	Warning("Increment!\n");
...
void CBasePlayer::PostThink()
{
	...
	if ( !g_fGameOver && !m_iPlayerLocked && IsAlive() && (GetTeamNumber() != 3) ) //LAWYER:  This doesn't apply to the Zombie Master.  I assume.
	{
...
void CBasePlayer::PostThinkVPhysics( void )
{
	...
	if ( GetMoveType() == MOVETYPE_NOCLIP || GetMoveType() == MOVETYPE_OBSERVER || GetMoveType() == MOVETYPE_CUSTOM )
	{
...
void CBasePlayer::InitialSpawn( void )
{
	...
	m_iZMPriority = 10;  //TGB: TODO: turn into cvar
}
...
void CBasePlayer::Spawn( void )
{
	...
	m_iZombiePool		= zm_initial_resources.GetInt(); //LAWYER:  Set up the zombie pool.
	m_iZombiePopCount	= 0;
	...
	m_iZombieSelected	= 0;
	m_fNextIncrement = 0; //LAWYER:  We want to increment ASAP
	...
	m_AirFinished	= gpGlobals->curtime + AIRTIME;
	m_nDrownDmgRate	= DROWNING_DAMAGE_INITIAL;
	m_bRoundRestart = 0; //qck: Init the variale, and respawn also marks end of round. 
 ...
 // only preserve the shadow flag
	...
	if (IsZM())
	{
		ShowViewPortPanel( PANEL_VIEWPORT, true ); //LAWYER:  show the Zombie panel!
	}
	else
	{
		ShowViewPortPanel( PANEL_VIEWPORT, false ); //LAWYER:  hide it!

	}
	//LAWYER:  Try to stop people from glitching water for infinite health
	m_bitsDamageType &= ~DMG_DROWNRECOVER;
	m_rgbTimeBasedDamage[itbd_DrownRecover] = 0;
	if ( IsPlayerUnderwater() == true )
	{
		SetPlayerUnderwater( false );
	}
...
int CBasePlayer::Restore( IRestore &restore )
{
	...
	QAngle newViewAngles = pl.v_angle.Get();	//TGB: 0000069 spectator first person fix
...
{
	if (GetTeamNumber() == TEAM_SPECTATOR)
		return; //LAWYER:  Spectators can't impulse
	...
	case	201:// paint decal
		...
		if ( gpGlobals->curtime < m_flNextDecalTime || GetTeamNumber() == 3)
		{
			// too early!
			//LAWYER:  ZombieMasters can't.
			break;
		}
...
bool CBasePlayer::ClientCommand(const char *cmd)
{
	...
	//TGB: zombie observe test
	else if ( stricmp( cmd, "spec_zombie" ) == 0 ) // chase zombie we're looking at
	{
		if ( GetObserverMode() > OBS_MODE_FIXED )
		{
			// find a zombie in front of us
			CBaseEntity *pEntity = FindPickerEntity( this );
			if ( pEntity && pEntity->IsNPC())
			{
				ForceObserverMode(OBS_MODE_IN_EYE);
				SetObserverTarget( pEntity );
			}
		}
...
bool CBasePlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	else 
	{
		...
		else
		{
			...
			if ( /*!PlayerHasMegaPhysCannon()*/ true ) //TGBMERGENOTE: we have no physcannon
			{
...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_SEND_TABLE_NOBASE(CPlayerState, DT_PlayerState)
		...
		SendPropQAngles (SENDINFO(v_angle), 13, SPROP_CHANGES_OFTEN), //TGB: 0000069 spectator first person fix; qck: Added SPROP_CHANGES_OFTEN to see if that might help the chop in first person spectating.
		//SendPropQAngles (SENDINFO(v_angle), 13), //qck: hlcoders first-person observe fix
	END_SEND_TABLE()
...
// If HL2_DLL is defined, then baseflex.cpp already sends these.
		...
		//LAWYER:  Additional crap to send
		//TGB: optimised a bit
		SendPropInt		(SENDINFO(m_iLastSelected), 32, SPROP_UNSIGNED), //Transmit the last selected entity index
		SendPropInt		(SENDINFO(m_iLastCost), 16, SPROP_UNSIGNED), //Transmit the last selected entity cost
		SendPropInt		(SENDINFO(m_iLastTrapCost), 16, SPROP_UNSIGNED), //Transmit the last selected trap cost
		SendPropInt		(SENDINFO(m_iLastZombieFlags), 16, SPROP_UNSIGNED), //Transmit the last selected entity flags
		SendPropInt		(SENDINFO(m_iZombiePool), 32, SPROP_UNSIGNED), //resources, I don't trust people to stay < 65k
		SendPropInt		(SENDINFO(m_iZombiePopCount), 32, SPROP_UNSIGNED), //count towards limit
		SendPropInt		(SENDINFO(m_iZombieSelected), 32, SPROP_UNSIGNED), //selected count
		SendPropInt		(SENDINFO(m_iZombieGroup)),
		SendPropBool	(SENDINFO( m_bCameraMode )),
		SendPropBool	(SENDINFO(m_bRoundRestart)),
		SendPropString	(SENDINFO(m_szLastDescription)), //qck edit
		SendPropStringT	(SENDINFO( m_ScreenEffect )), //qck: screen space effects
...
void CBasePlayer::VPhysicsShadowUpdate( IPhysicsObject *pPhysics )
{
	...
	if ( GetMoveType() == (MOVETYPE_NOCLIP | MOVETYPE_CUSTOM) )
	{
		m_oldOrigin = GetAbsOrigin();
		return;
	}
...
CBasePlayer *CBasePlayer::GetZM()
{
	...
	return NULL; //uh oh, no ZM
}

//-----------------------------------------------------------------------------
// Purpose: Gives an importance level.  0 is null, 1 is the highest, all beyond that are decreasing importance.
//-----------------------------------------------------------------------------
int CBasePlayer::GetZMImportance( void )
{ 
	//TGB: removed for source release, best not to have your steamid scattered around too much
	return 0;
}

//--------------------------------------------------------------
// TGB: recalculate the population and write it to the popcount
//--------------------------------------------------------------
void CBasePlayer::ZM_RecalcPop()
{
	int pop = 0;
	...
	{
		CBaseEntity *pEnt = gEntList.m_ZombieList[i];
		if (!pEnt)
			continue;

		CNPC_BaseZombie *pZombie = dynamic_cast<CNPC_BaseZombie *>(pEnt);

		if (pZombie)
		{
			pop += pZombie->GetPopCost();
		}
	}

	m_iZombiePopCount.Set(pop);
	//return pop;
}
