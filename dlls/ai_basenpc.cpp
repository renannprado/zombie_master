...
ConVar	ai_default_efficient( "ai_default_efficient", ( /*IsXbox()*/ true ) ? "1" : "0" ); //TGB: efficiency is good
...
Activity CAI_BaseNPC::GetFlinchActivity( bool bHeavyDamage, bool bGesture )
{
	...
	//TGB: we don't have non-gesture flinches
	bGesture = true;
...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	//LAWYER:  Extra code added for additional points!
/*	if (info.GetAttacker())
	{
			CBasePlayer *pKiller = ToBasePlayer( info.GetAttacker() ); //Might fix the firebug
			
			if (pKiller)
			{
				pKiller->IncrementFragCount(1); //LAWYER:  One point per zombie kill!
			}
		
	}*/ //LAWYER:  Killing Zombies no longer awards points
}
...
{
#ifdef HL2_DLL
	//TGB: we've excluded bullseyes from our projfile, so comment this to avoid undefined symbols of it
...
CBaseEntity *CAI_BaseNPC::EyeLookTarget( void )
{
	if (m_flNextEyeLookTime < gpGlobals->curtime)
	{
		...
		for ( CEntitySphereQuery sphere( GetAbsOrigin(), 1024, 0 ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
		{
			...
			{
				//TGB: never look at players that are not survivor
				if ( pEntity->IsPlayer() && pEntity->GetTeamNumber() != 2 )
					continue;
...
void CAI_BaseNPC::RunAnimation( void )
{
	...
	if ( m_NPCState != NPC_STATE_SCRIPT && m_NPCState != NPC_STATE_DEAD && m_Activity == ACT_IDLE && IsActivityFinished() )
	{
		...
		if ( iSequence != ACTIVITY_NOT_AVAILABLE )
		{
			...
			{
				m_nIdealSequence = iSequence; //TGB: laggy anim fix? //upd: nope
			}
		}
	}
...
void CAI_BaseNPC::UpdateEfficiency( bool bInPVS )
{
	...
	//TGBanimdebug: not related to problem
	/*TGB: we can't really afford anything except AIME_EFFICIENT
...
void CAI_BaseNPC::NPCThink( void )
{
	...
	if(m_bRallyPointSeek)
		SearchOutRallyPoint(); //qck: I'm not sure this is the best place for this...
...
void CAI_BaseNPC::SetupVPhysicsHull()
{
	...
	if ( pPhysObj )
	{
		...
		if (modelptr != NULL) //TGB: it crashed due to this being NULL once...
		{
			float mass = Studio_GetMass(modelptr);
			if ( mass > 0 )
			{
			...
			pPhysObj->SetMass( 50 ); //TGB: better than nothing I guess
...
void CAI_BaseNPC::StartNPC( void )
{
	...
	//Msg("NPC spawn point: %i \n", m_iOwner); 

	m_bRallyPointSeek = true; //qck: Edit: We were just spawned so we should seek out any rally points. If there are none? Do whatever. 
	
}


//-------------------------------
//qck: Search out our rally point
//-------------------------------
...
void CAI_BaseNPC::HandleAnimEvent( animevent_t *pEvent )
{
	...
	default:
		if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER|AE_TYPE_SHARED))
		{
...
// Output :
//-----------------------------------------------------------------------------
//TGBMERGENOTE: compile error and we don't need it
...
//	lagcompensation->RemoveNpcData( GetRefEHandle() );

	//TGB: lagcomp stuff that's stored as member is destructed automatically here
...
#endif // HL2_DLL
*/ //LAWYER:  No need, due to distinct lack of Barnacle.
...
//qck: Method definitions
void CAI_BaseNPC::SetRallySearch( bool search)
{
	m_bShouldSearchForRallyPoint = search;
}

void CAI_BaseNPC::SetZombieSpawnID( int ID )
{
	m_iOwner = ID;
}
