...
BEGIN_DATADESC( CEnvSpark )
	...
	//TGB
	DEFINE_KEYFIELD( m_flDeathTime, FIELD_INTEGER, "DeathTime" ),
...
void CEnvSpark::Spawn(void)
{
	...
	if (m_flDeathTime <= 0)
		m_flDeathTime = -1;
	else if ( m_flDeathTime < gpGlobals->curtime )
		m_flDeathTime = gpGlobals->curtime + 7.f;
...
{
	//TGB
	if (m_flDeathTime > 0 && m_flDeathTime < gpGlobals->curtime)
	{
		DevMsg("Killing Sparker at %f\n", gpGlobals->curtime);
		inputdata_t temp;
		InputKill( temp );
	}
