...
CGlobalEntityList::CGlobalEntityList()
{
	...
	//TGB: clean up zombie lists just to make sure

	m_ZombieList.Purge();
	m_BansheeList.Purge();
	m_ZombieSelected.Purge();
	m_ZombieSpawns.Purge();
	m_ZombieManipulates.Purge();
...
//--------------------------------------------------------------
// TGB: tests for NULL entities in the list and reports their presence
//--------------------------------------------------------------
void CGlobalEntityList::TestEntities()
{
	//loop ripped from Find* funcs
	const CEntInfo *pInfo = FirstEntInfo();
	for ( ;pInfo; pInfo = pInfo->m_pNext )
	{
		CBaseEntity *ent = (CBaseEntity *)pInfo->m_pEntity;
		if ( !ent )
		{
			DevWarning( "NULL entity in global entity list @ %i\n", pInfo->m_SerialNumber );
			continue;
		}
	}
}
...
static ConCommand restart_entities( "respawn_entities", RespawnEntities, "Respawn all the entities in the map.", FCVAR_CHEAT ); //LAWYER:  Encheatened, because people can crash the game with it.
