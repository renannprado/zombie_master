...
void CServerGameDLL::LevelInit_ParseAllEntities( const char *pMapEntities )
{
	...
	MapEntity_ParseAllEntities( pMapEntities, &filter ); //LAWYER: Updated by valve - might not work
	//TGB: ZM now requires the above as well due to the use of a HL2MP-like filter in WorldReset()

	// TR Game Specific code.
	// This allows us to "Cache" a copy of MapEntities which we need
	// so we can reset the world. r00t
	//LAWYER:  More beer is owed
	//((CHL2MPRules*)g_pGameRules)->SetMapEntityCache(pMapEntities); //LAWYER:  Might need to be recovered
