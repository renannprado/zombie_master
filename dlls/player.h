...
//	virtual CBaseCombatWeaponHandle			Weapon_GetLast(); //LAWYER:  Hack around to get the last weapon
	...
	virtual CBaseCombatWeapon *GetLastWeapon( void ) { return m_hLastWeapon.Get(); } //LAWYER:  Transposed, for great justice
	...
	//TGB: ZM check, see baseplayer_shared.cpp
	bool					IsZM();
	bool					IsSurvivor();
	static CBasePlayer		*GetZM(); //finds the ZM player and returns a ptr


	//TGB: recalculate zombie population and save to the var
	void						ZM_RecalcPop();
...
public:
	...
	//LAWYER:  Transmitted data stuff
	CNetworkVar( int, m_iLastSelected );//LAWYER:  The last entity selected by the player
	CNetworkVar( int, m_iLastCost );//LAWYER:  The last entity selected by the player
	CNetworkVar( int, m_iLastTrapCost );
	CNetworkVar( int, m_iZombiePool );//LAWYER:  The zombie pool!
	CNetworkVar( int, m_iZombiePopCount ); //TGB
	CNetworkVar( int, m_iZombieSelected ); //TGB
	CNetworkVar( int, m_iZombieGroup ); //qck
	CNetworkVar( bool, m_bRoundRestart ); //qck edits
	CNetworkString(m_szLastDescription, 255); //qck edits
	CNetworkVar( int, m_iLastZombieFlags );//LAWYER:  The Zombie Flags property of the last spawn selected
	CNetworkVar(string_t, m_ScreenEffect); //qck: screen effect stuff (shaders)

    void SetScreenEffect(string_t screenEffect) 
	{
		m_ScreenEffect = screenEffect;
	}

	string_t GetScreenEffect()
	{
		string_t effect = m_ScreenEffect;
		return effect;
	}

//	CZombieListManager	m_pZombieListManager;
	//---------------------------------
	// Inputs
	//---------------------------------
...
public:
	...
	CNetworkVar( bool, m_bCameraMode );; //LAWYER:
	...
	bool					m_bHoldingObject;    //qck: Toggle to let the rest of the world know we're holding something 
	
	CUtlVector< int	>		m_ZombieGroupHandles; //qck: Used for final transfer from client->server
	int						m_iOldZombieGroup;


	float		GetDeathTime( void ) { return m_flDeathTime; }
	float					m_fNextIncrement;				// LAWYER: the next time resources should be incremented
private:
...
protected:
	...
	//TGB ZM priority
public:
	int					GetZMPriority() { return m_iZMPriority; }
	int					m_iZMPriority; //used to be private
	//LAWYER:  Round restart willpower
	int					GetZMVoteRR() { return m_iZMVoteRR; }
	int					m_iZMVoteRR;
	int					GetZMImportance();
